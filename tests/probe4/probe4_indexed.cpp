...
static bool readbackHasQuadPixels(ID3D12Resource* readback, UINT rowPitch) {
    void* mapped = nullptr;
    ...
    if (chromaPixels < 500) {
        fprintf(stderr, "[FAIL] Pixel readback did not find rendered indexed quad color\n");
        return false;
    }
    fprintf(stdout, "[PASS] Pixel readback confirmed rendered indexed quad color\n");
    return true;
}
...
int main() {
    ...
    bool pixelPass = readbackHasQuadPixels(readback, readbackPitch);
