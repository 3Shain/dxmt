...
struct DefineUseChainEntry {
  DefineUseChainEntry()
      : psInst(0), psOp(0), usages(), writeMask(0), index(0), isStandalone(0) {
  ...
  uint32_t writeMask;    // Access mask; which all components were written to in
                         // the same op
  uint32_t index;        // For which component was this definition created for?
  uint32_t isStandalone; // A shortcut for analysis: if nonzero, all siblings of
                         // all usages for both this and all this siblings
  struct DefineUseChainEntry *
      psSiblings[4]; // In case of vectorized op, contains pointer to this
                     // define's corresponding entries for the other components.
...
struct UseDefineChainEntry {
  UseDefineChainEntry()
      : psInst(0), psOp(0), defines(), accessMask(0), index(0) {
  ...
  struct UseDefineChainEntry
      *psSiblings[4]; // In case of vectorized op, contains pointer to this
                      // usage's corresponding entries for the other components.
...
void BuildUseDefineChains(std::vector<Instruction> &instructions,
                          uint32_t ui32NumTemps, DefineUseChains &psDUChains,
...
// Optimization pass for successive passes: Mark Operand->isStandalone for
// definitions that are "standalone": all usages (and all their sibligns) of
// this and all its siblings only see this definition.
