...
uint32_t Shader::GetTempComponentCount(SHADER_VARIABLE_TYPE eType,
...
void Shader::ConsolidateHullTempVars() {
  ...
  for (phase = 0; phase < phases.size(); phase++) {
    for (i = 0; i < phases[phase].psDecl.size(); i++) {
      if (phases[phase].psDecl[i].eOpcode == OPCODE_DCL_TEMPS) {
        if (phases[phase].psDecl[i].value.ui32NumTemps > numTemps)
          numTemps = phases[phase].psDecl[i].value.ui32NumTemps;
        phases[phase].psDecl[i].value.ui32NumTemps = 0;
      }
    }
  }
  ...
  for (phase = 0; phase < phases.size(); phase++) {
    for (i = 0; i < phases[phase].psDecl.size(); i++) {
      if (phases[phase].psDecl[i].eOpcode == OPCODE_DCL_TEMPS) {
        phases[phase].psDecl[i].value.ui32NumTemps = numTemps;
        return;
      }
    }
  }
...
// Image (RWTexture in HLSL) declaration op does not provide enough info about
// the format and accessing. Go through all image declarations and instructions
// accessing it to see if it is readonly/writeonly. While doing that we also get
// the number of components expected in the image format. Also resolve access
...
void ShaderPhase::ResolveUAVProperties(const ShaderInfo &sInfo) {
  ...
  for (psDecl = psFirstDeclaration; psDecl <= psLastDeclaration; psDecl++) {
    ...
    uavReg = psDecl->operands[0].ui32RegisterNumber;
    ...
    for (psInst = psFirstInstruction; psInst <= psLastInstruction; psInst++) {
      ...
      case OPCODE_LD_UAV_TYPED:
        ...
        numComponents = psInst->operands[0]
                            .GetNumSwizzleElements(); // get component count
                                                      // from the write target
        break;

      case OPCODE_STORE_UAV_TYPED:
        ASSERT(psInst->operands[0].eType == OPERAND_TYPE_UNORDERED_ACCESS_VIEW);
        opIndex = 0;
        accessFlags = ACCESS_FLAG_WRITE;
        numComponents =
            0; // store op does not contribute on the component count resolving
        break;
      ...
      // The rest of the ops here are only for buffer UAVs. No need for
      // component count resolving.
      ...
      if (psInst->operands[opIndex].eType != OPERAND_TYPE_UNORDERED_ACCESS_VIEW)
        continue;

      // Check the instruction is operating on the declared uav
      if (psInst->operands[opIndex].ui32RegisterNumber != uavReg)
        continue;

      psDecl->sUAV.ui32AccessFlags |= accessFlags;

      // get the max components accessed, but only for typed (texture) UAVs
      if (numComponents > psDecl->sUAV.ui32NumComponents &&
          psDecl->eOpcode == OPCODE_DCL_UNORDERED_ACCESS_VIEW_TYPED) {
    ...
    if (psDecl->eOpcode == OPCODE_DCL_UNORDERED_ACCESS_VIEW_TYPED) {
      ...
      if (sInfo.GetResourceFromBindingPoint(RGROUP_UAV, uavReg, &psBinding)) {
        // component count is stored in flags as 2 bits, 00: vec1, 01: vec2, 10:
        // vec3, 11: vec4
        psDecl->sUAV.ui32NumComponents = ((psBinding->flags >> 2) & 3) + 1;
      }
    }
  }
}

static void GatherOperandAccessMasks(const Operand *psOperand,
                                     char *destTable) {
  int i;
  uint32_t reg;
  ...
  reg = psOperand->ui32RegisterNumber &
        0xffff; // We add 0x10000 to all newly created ones earlier

  destTable[reg] |= (char)psOperand->GetAccessMask();
}

// Coalesce the split temps back based on their original temp register. Keep
// uint/int/float operations separate
static void CoalesceTemps(Shader *psShader, ShaderPhase *psPhase,
                          uint32_t ui32MaxOrigTemps) {
  // Just move all operations back to their original registers, but keep the
  // data type assignments.
  uint32_t i, k;
  Instruction *psLastInstruction = &psPhase->psInst[psPhase->psInst.size() - 1];
  std::vector<char> opAccessMasks;

  // First move all newly created temps to high enough so they won't overlap
  // with the rebased ones
  ...
  while (inst <= psLastInstruction) {
    ...
    for (i = psPhase->ui32OrigTemps; i < psPhase->ui32TotalTemps; i++) {
      for (k = 0; k < inst->ui32NumOperands; k++)
        inst->ChangeOperandTempRegister(&inst->operands[k], i, 0x10000 + i,
  ...
  while (inst <= psLastInstruction) {
    for (k = 0; k < inst->ui32NumOperands; k++)
      GatherOperandAccessMasks(&inst->operands[k], &opAccessMasks[0]);
    inst++;
  }
  ...
  for (i = 0; i < psPhase->ui32TotalTemps; i++) {
    ...
    if (needsMoving) {
      ...
      while (inst <= psLastInstruction) {
        for (k = 0; k < inst->ui32NumOperands; k++)
          inst->ChangeOperandTempRegister(&inst->operands[k], origReg, newReg,
                                          OPERAND_4_COMPONENT_MASK_ALL,
                                          UD_CHANGE_ALL, rebase);
        inst++;
      }
    }
    // Mark the count
    ...
    default:
      ...
      break;
    case SVT_BOOL:
      psShader->psBoolTempSizes[newReg] =
          std::max(psShader->psBoolTempSizes[newReg], (char)count);
      break;
    case SVT_FLOAT:
      psShader->psFloatTempSizes[newReg] =
          std::max(psShader->psFloatTempSizes[newReg], (char)count);
      break;
    case SVT_FLOAT16:
      psShader->psFloat16TempSizes[newReg] =
          std::max(psShader->psFloat16TempSizes[newReg], (char)count);
      break;
    case SVT_FLOAT10:
      psShader->psFloat10TempSizes[newReg] =
          std::max(psShader->psFloat10TempSizes[newReg], (char)count);
      break;
    case SVT_INT:
      psShader->psIntTempSizes[newReg] =
          std::max(psShader->psIntTempSizes[newReg], (char)count);
      break;
    case SVT_INT16:
      psShader->psInt16TempSizes[newReg] =
          std::max(psShader->psInt16TempSizes[newReg], (char)count);
      break;
    case SVT_INT12:
      psShader->psInt12TempSizes[newReg] =
          std::max(psShader->psInt12TempSizes[newReg], (char)count);
      break;
    case SVT_UINT:
      psShader->psUIntTempSizes[newReg] =
          std::max(psShader->psUIntTempSizes[newReg], (char)count);
      break;
    case SVT_UINT16:
      psShader->psUInt16TempSizes[newReg] =
          std::max(psShader->psUInt16TempSizes[newReg], (char)count);
      break;
    case SVT_DOUBLE:
      psShader->psDoubleTempSizes[newReg] =
          std::max(psShader->psDoubleTempSizes[newReg], (char)count);
      break;
    }
  }
}
...
void Shader::PruneTempRegisters() {
  ...
  for (k = 0; k < phases.size(); k++) {
    ShaderPhase *psPhase = &phases[k];
  ...
  // Allocate and zero-initialize arrays for each temp sizes. *4 is for every
  // possible rebase
...
static void
DoSignatureAnalysis(std::vector<ShaderInfo::InOutSignature> &psSignatures,
                    std::vector<unsigned char> &outTable) {
  // Fill the char, 2 bits per component so that each 2 bits encode the
  // following info: 0: unused OR used by the first signature we happened to see
  // 1: used by the second signature
  // 2: used by the third sig
  // 3: used by the fourth sig.

  // The counters for each input/output/patch. Start with 8 registers, grow as
  // needed
  ...
  for (i = 0; i < psSignatures.size(); i++) {
    ...
    currCounter = currCounter | (currCounter << 2) | (currCounter << 4) |
                  (currCounter << 6);
    // Widen the mask
    mask = (unsigned char)psSig->ui32Mask;
    mask =
        ((mask & 8) << 3) | ((mask & 4) << 2) | ((mask & 2) << 1) | (mask & 1);
    mask = mask | (mask << 1);
    // Write output
...
void Shader::DoIOOverlapOperand(ShaderPhase *psPhase, Operand *psOperand) {
  ...
  case OPERAND_TYPE_INPUT_PATCH_CONSTANT:
    redirectTable = regSpace == 0 ? &psPhase->acInputNeedsRedirect[0]
                                  : &psPhase->acPatchConstantsNeedsRedirect[0];
    break;

  case OPERAND_TYPE_OUTPUT:
  case OPERAND_TYPE_OUTPUT_CONTROL_POINT:
    redirectTable = regSpace == 0 ? &psPhase->acOutputNeedsRedirect[0]
                                  : &psPhase->acPatchConstantsNeedsRedirect[0];
    break;
...
void Shader::AnalyzeIOOverlap() {
  ...
  DoSignatureAnalysis(info.inputSignatures, outData);

  // Now data has the values, copy them to all phases
  for (i = 0; i < phases.size(); i++)
    phases[i].acInputNeedsRedirect = outData;

  DoSignatureAnalysis(info.outputSignatures, outData);
  for (i = 0; i < phases.size(); i++)
    phases[i].acOutputNeedsRedirect = outData;

  DoSignatureAnalysis(info.patchConstantSignatures, outData);
  for (i = 0; i < phases.size(); i++)
    phases[i].acPatchConstantsNeedsRedirect = outData;

  // Now walk through all operands and suboperands in all instructions and write
  // 0xff to the dest (cannot occur otherwise) if we're crossing signature
  // borders
  for (i = 0; i < phases.size(); i++) {
    ShaderPhase *psPhase = &phases[i];
    for (k = 0; k < psPhase->psInst.size(); k++) {
      Instruction *psInst = &psPhase->psInst[k];
      uint32_t j;
      for (j = 0; j < psInst->ui32NumOperands; j++)
        DoIOOverlapOperand(psPhase, &psInst->operands[j]);
    }

    // Now prune all tables from anything except 0xff.
    std::for_each(psPhase->acInputNeedsRedirect.begin(),
                  psPhase->acInputNeedsRedirect.end(), PruneRedirectEntry);
    std::for_each(psPhase->acOutputNeedsRedirect.begin(),
                  psPhase->acOutputNeedsRedirect.end(), PruneRedirectEntry);
    std::for_each(psPhase->acPatchConstantsNeedsRedirect.begin(),
                  psPhase->acPatchConstantsNeedsRedirect.end(),
                  PruneRedirectEntry);
  }
}

void Shader::SetMaxSemanticIndex() {
  for (std::vector<ShaderInfo::InOutSignature>::iterator it =
           info.inputSignatures.begin();
       it != info.inputSignatures.end(); ++it)
    maxSemanticIndex = std::max(maxSemanticIndex, it->semanticIndex);

  for (std::vector<ShaderInfo::InOutSignature>::iterator it =
           info.outputSignatures.begin();
       it != info.outputSignatures.end(); ++it)
    maxSemanticIndex = std::max(maxSemanticIndex, it->semanticIndex);

  for (std::vector<ShaderInfo::InOutSignature>::iterator it =
           info.patchConstantSignatures.begin();
       it != info.patchConstantSignatures.end(); ++it)
    maxSemanticIndex = std::max(maxSemanticIndex, it->semanticIndex);
}

// In DX bytecode, all const arrays are vec4's, and all arrays are stuffed to
// one large array. Luckily, each chunk is always accessed with suboperand plus
// <constant> (in ui32RegisterNumber) So do an analysis pass. Also trim the
// vec4's into smaller formats if the extra components are never read.
void ShaderPhase::PruneConstArrays() {
  auto customDataItr =
      find_if(psDecl.begin(), psDecl.end(), [](const Declaration &d) {
        return d.eOpcode == OPCODE_CUSTOMDATA;
      });
  ...
  dxmt::ForEachOperand(
      psInst.begin(), psInst.end(), FEO_FLAG_ALL,
      [this](const std::vector<Instruction>::iterator &psInst,
             const Operand *psOperand, uint32_t ui32OperandType) {
        if (psOperand->eType == OPERAND_TYPE_IMMEDIATE_CONSTANT_BUFFER) {
          ...
          // Find all existing entries that have anything common with the access
          // mask
          auto cbrange = m_ConstantArrayInfo.m_Chunks.equal_range(offset);
          std::vector<ChunkMap::iterator> matchingEntries;
          ...
          if (matchingEntries.empty()) {
            ...
            m_ConstantArrayInfo.m_Chunks.insert(std::make_pair(
                offset,
                ConstantArrayChunk(0u, accessMask, (Operand *)psOperand)));
            ...
            matchingEntries[0]->second.m_UseSites.push_back(
                (Operand *)psOperand);
          } else {
            // Multiple entries with (now) overlapping mask. Merge to the first
            // one.
            ...
            for_each(matchingEntries.begin() + 1, matchingEntries.end(),
                     [&tgt, &chunks](ChunkMap::iterator itr) {
  ...
  // Figure out how large each chunk is by finding the next chunk that uses any
  // bits from the current mask (or the max size if not found)

  uint32_t totalSize =
      (uint32_t)
          m_ConstantArrayInfo.m_OrigDeclaration->asImmediateConstBuffer.size();
  for (auto chunk = m_ConstantArrayInfo.m_Chunks.begin();
       chunk != m_ConstantArrayInfo.m_Chunks.end(); chunk++) {
    // Find the next chunk that shares any bits in the access mask
    auto nextItr = find_if(
        m_ConstantArrayInfo.m_Chunks.lower_bound(chunk->first + 1),
        m_ConstantArrayInfo.m_Chunks.end(),
        [&chunk](ChunkMap::value_type &itr) {
    ...
    for_each(chunk->second.m_UseSites.begin(), chunk->second.m_UseSites.end(),
             [&rebase, &componentCount](Operand *op) {
               // Store the rebase value to each operand and do the actual
               // rebase.
               op->m_Rebase = rebase;
               op->m_Size = componentCount;

               if (rebase != 0) {
                 // Update component mask. Note that we don't need to do
                 // anything to the suboperands. They do not affect destination
                 // writemask.
                 switch (op->eSelMode) {
                 ...
                 case OPERAND_4_COMPONENT_SWIZZLE_MODE: {
                   for (int i = 0; i < 4; i++) {
                     // Note that this rebase is different from the one done for
                     // source operands
  ...
  // We'll do the actual declaration and pruning later on, now that we have the
  // info stored up.
}

dxmt::ControlFlow::ControlFlowGraph &ShaderPhase::GetCFG() {
  if (!m_CFGInitialized) {
...
void ShaderPhase::UnvectorizeImmMoves() {
  // NOTE must be called before datatype analysis and other analysis phases are
  // done, as the pointers won't match anymore (we insert new instructions
  // there)
  std::vector<Instruction> nInst;
  // Reserve 1.5x space
  nInst.reserve(psInst.size() * 3 / 2);

  for_each(psInst.begin(), psInst.end(), [&](Instruction &i) {
    if (i.eOpcode != OPCODE_MOV || i.operands[0].eType != OPERAND_TYPE_TEMP ||
        i.operands[1].eType != OPERAND_TYPE_IMMEDIATE32 ||
        i.operands[0].GetNumSwizzleElements() == 1) {
      nInst.push_back(i);
      return;
    }
    // Ok, found one to unvectorize.
    ASSERT(i.operands[0].eSelMode == OPERAND_4_COMPONENT_MASK_MODE);
    uint32_t mask = i.operands[0].ui32CompMask;
    for (uint32_t j = 0; j < 4; j++) {
      if ((mask & (1 << j)) == 0)
        continue;

      Instruction ni = i;
      ni.operands[0].ui32CompMask = (1 << j);
      nInst.push_back(ni);
    }
  });
  psInst.clear();
...
void ShaderPhase::ExpandSWAPCs() {
  ...
  auto dcitr = std::find_if(psDecl.begin(), psDecl.end(),
                            [](const Declaration &decl) -> bool {
                              return decl.eOpcode == OPCODE_DCL_TEMPS;
                            });
  if (dcitr == psDecl.end()) {
  ...
  while (1) {
    // Need to find from top every time, because we're inserting stuff into the
    // vector
    auto swapItr = std::find_if(psInst.begin(), psInst.end(),
                                [](const Instruction &inst) -> bool {
                                  return inst.eOpcode == OPCODE_SWAPC;
                                });
    if (swapItr == psInst.end())
      break;

    // Ok swapItr now points to a SWAPC instruction that we'll have to split up
    // like this (from MSDN):
    ...
    */
    ...
    swapItr->operands[0] = origSwapInst.operands[0];
    swapItr->operands[0].eType = OPERAND_TYPE_TEMP;
    swapItr->operands[0].ui32RegisterNumber = extraTemp;
    // mask is already fine
    swapItr->operands[1] = origSwapInst.operands[2]; // src0
    swapItr->operands[2] = origSwapInst.operands[4]; // src2
    swapItr->operands[3] = origSwapInst.operands[3]; // src1
    ...
    newInst[0].operands[0] = origSwapInst.operands[1]; // dest1
    newInst[0].operands[1] = origSwapInst.operands[2]; // src0
    newInst[0].operands[2] = origSwapInst.operands[3]; // src1
    newInst[0].operands[3] = origSwapInst.operands[4]; // src2
#if _DEBUG
    ...
    newInst[1].operands[0] = origSwapInst.operands[0]; // dest 0
    // First copy dest0 to src as well to get the mask set up correctly
    newInst[1].operands[1] = origSwapInst.operands[0]; // dest 0;
    // Then overwrite with temp reg
    newInst[1].operands[1].eType = OPERAND_TYPE_TEMP;
    newInst[1].operands[1].ui32RegisterNumber = extraTemp;
#if _DEBUG
...
void Shader::ExpandSWAPCs() {
  ...
  for (int i = 0; i < phases.size(); i++) {
    phases[i].ExpandSWAPCs();
  }
}

void Shader::ForcePositionToHighp() {
  // Only sensible in vertex shaders (TODO: is this an issue in tessellation
  // shaders? Do we even care?)
  if (eShaderType != VERTEX_SHADER)
    return;

  ShaderPhase &phase = phases[0];

  // Find the output declaration
  std::vector<Declaration>::iterator itr = std::find_if(
      phase.psDecl.begin(), phase.psDecl.end(),
      [this](const Declaration &decl) -> bool {
        if (decl.eOpcode == OPCODE_DCL_OUTPUT_SIV) {
          const SPECIAL_NAME specialName = decl.operands[0].eSpecialName;
          if (specialName == NAME_POSITION ||
              specialName == NAME_UNDEFINED) // This might be SV_Position
                                             // (because d3dcompiler is weird).
          {
            const ShaderInfo::InOutSignature *sig = NULL;
            info.GetOutputSignatureFromRegister(
                decl.operands[0].ui32RegisterNumber,
                decl.operands[0].GetAccessMask(), 0, &sig);
            ASSERT(sig != NULL);
            if ((sig->systemValueType == NAME_POSITION ||
                 sig->semanticName == "POS") &&
                sig->semanticIndex == 0) {
              ((ShaderInfo::InOutSignature *)sig)->minPrecision =
                  MIN_PRECISION_DEFAULT;
              return true;
            }
          }
          return false;
  ...
  // Do nothing if we don't find suitable output. This may well be
  // INTERNALTESSPOS for tessellation etc.
  if (itr == phase.psDecl.end())
    return;

  uint32_t outputPosReg = itr->operands[0].ui32RegisterNumber;

  dxmt::ForEachOperand(phase.psInst.begin(), phase.psInst.end(),
                         FEO_FLAG_DEST_OPERAND,
                         [outputPosReg](std::vector<Instruction>::iterator itr,
                                        Operand *op, uint32_t flags) {
                           if (op->eType == OPERAND_TYPE_OUTPUT &&
                               op->ui32RegisterNumber == outputPosReg)
  ...
  for (int i = 0; i < phases.size(); i++) {
    ...
    dxmt::ForEachOperand(phase.psInst.begin(), phase.psInst.end(),
                           ...
                           [&](std::vector<Instruction>::iterator inst,
                             ...
                             const uint32_t ui32BindingPoint =
                                 op->aui32ArraySizes[0];
                             const ConstantBuffer *psCBuf = NULL;
                             info.GetConstantBufferFromBindingPoint(
                                 RGROUP_CBUFFER, ui32BindingPoint, &psCBuf);

                             if (!psCBuf)
                               return;

                             // Get all the struct members that can be reached
                             // from this usage:
                             ...
                             for (uint32_t k = 0; k < 4; k++) {
                               ...
                               ShaderInfo::GetShaderVarFromOffset(
                                   op->aui32ArraySizes[1], tmpSwizzle, psCBuf,
                                   (const ShaderVarType **)&psVarType, &isArray,
                                   NULL, &rebase, flags);
...
HLSLCC_API char *TranslateToMSL(DXBCShader *_shader) {
  char *glslcstr = NULL;
  int success = 0;
  ...
  // if (!CheckConstantBuffersNoDuplicateNames(shader->info.constantBuffers,
  //                                           reflectionCallbacks))
  //   return 0;
...
HLSLCC_API void GetDXBCShaderInfo(DXBCShader *_shader,
