...
#define FEO_FLAG_ALL                                                           \
  (FEO_FLAG_SUBOPERAND | FEO_FLAG_SRC_OPERAND | FEO_FLAG_DEST_OPERAND)
      ...
      for (i = 0; i < inst->ui32FirstSrc; i++) {
        if (flags & FEO_FLAG_SUBOPERAND) {
          for (k = 0; k < MAX_SUB_OPERANDS; k++) {
            if (inst->operands[i].m_SubOperands[k].get()) {
              callback(inst, inst->operands[i].m_SubOperands[k].get(),
                       FEO_FLAG_SUBOPERAND);
            }
          }
        }
        if (flags & FEO_FLAG_DEST_OPERAND) {
          callback(inst, &inst->operands[i], FEO_FLAG_DEST_OPERAND);
        }
      }
    }
    ...
    if ((flags & FEO_FLAG_SRC_OPERAND) || (flags & FEO_FLAG_SUBOPERAND)) {
      for (i = inst->ui32FirstSrc; i < inst->ui32NumOperands; i++) {
        ...
        if (flags & FEO_FLAG_SRC_OPERAND) {
          callback(inst, &inst->operands[i], FEO_FLAG_SRC_OPERAND);
        }
      }
    }
