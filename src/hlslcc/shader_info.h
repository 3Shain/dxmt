...
typedef enum _SHADER_VARIABLE_TYPE {
  ...
  SVT_FORCED_INT = 152,
  // Integer that can be either signed or unsigned. Only used as an intermediate
  // step when doing data type analysis
...
enum ResourceGroup {
  RGROUP_CBUFFER, // ConstantBuffer
  RGROUP_TEXTURE, // ShaderResource
  RGROUP_SAMPLER, // SamplerState
  RGROUP_UAV,     // UnorderedAceessView
  RGROUP_COUNT,
};
...
struct ResourceBinding {
  ...
  uint32_t bindCount;
  uint32_t flags;
  uint32_t reigsterSpace;
  uint32_t rangeId;
  REFLECT_RESOURCE_DIMENSION resourceDimension;
  RESOURCE_RETURN_TYPE resourceReturnType;
  uint32_t numSamples;
  int m_SamplerMode; // (SB_SAMPLER_MODE) For samplers, this is the sampler mode
                     // this sampler is declared with
...
struct ShaderVarType {
  ShaderVarType()
      : Class(), Type(), Rows(), Columns(), Elements(), MemberCount(), Offset(),
        ParentCount(), Parent(), m_IsUsed(false) {}
...
public:
  struct InOutSignature {
    ...
    SPECIAL_NAME systemValueType;
    INOUT_COMPONENT_TYPE eComponentType;
    uint32_t ui32Register;
    uint32_t ui32Mask;
    uint32_t ui32ReadWriteMask;

    int iRebase; // If mask does not start from zero, this indicates the offset
                 // that needs to be subtracted from each swizzle

    uint32_t ui32Stream;
    MIN_PRECISION minPrecision;

    std::set<uint32_t> isIndexed; // Set of phases where this input/output is
                                  // part of a index range.
  ...
  ShaderInfo()
      : ui32MajorVersion(), ui32MinorVersion(), resouceBindings(),
        constantBuffers(), psThisPointerConstBuffer(), psClassTypes(),
        psClassInstances() {}

  SHADER_VARIABLE_TYPE GetTextureDataType(uint32_t regNo);

  int GetResourceFromBindingPoint(const ResourceGroup eGroup,
                                  const uint32_t ui32BindPoint,
                                  const ResourceBinding **ppsOutBinding) const;

  void
  GetConstantBufferFromBindingPoint(const ResourceGroup eGroup,
                                    const uint32_t ui32BindPoint,
                                    const ConstantBuffer **ppsConstBuf) const;

  int GetInterfaceVarFromOffset(uint32_t ui32Offset,
                                ShaderVar **ppsShaderVar) const;

  int GetInputSignatureFromRegister(const uint32_t ui32Register,
                                    const uint32_t ui32Mask,
                                    const InOutSignature **ppsOut,
                                    bool allowNull = false) const;
  int GetPatchConstantSignatureFromRegister(const uint32_t ui32Register,
                                            const uint32_t ui32Mask,
                                            const InOutSignature **ppsOut,
                                            bool allowNull = false) const;
  ...
  int GetOutputSignatureFromSystemValue(SPECIAL_NAME eSystemValueType,
                                        uint32_t ui32SemanticIndex,
                                        const InOutSignature **ppsOut) const;

  static ResourceGroup ResourceTypeToResourceGroup(ResourceType);

  static uint32_t GetCBVarSize(const ShaderVarType *psType,
                               bool matrixAsVectors,
                               bool wholeArraySize = false);
  ...
  static int GetShaderVarFromOffset(const uint32_t ui32Vec4Offset,
                                    ...
                                    int32_t *pi32Rebase, uint32_t flags);

  static std::string GetShaderVarIndexedFullName(
  ...
  std::vector<InOutSignature> inputSignatures;
  std::vector<InOutSignature> outputSignatures;
  std::vector<InOutSignature> patchConstantSignatures;

  std::vector<ResourceBinding> resouceBindings;
  ...
  dxmt::growing_vector<uint32_t> aui32TableIDToTypeID;

  dxmt::growing_vector<uint32_t> aui32ResourceMap[RGROUP_COUNT];

  dxmt::growing_vector<ShaderVarType> sGroupSharedVarType;
  ...
  union {
    ...
    struct {
      TESSELLATOR_PARTITIONING TessPartitioning;
      TESSELLATOR_OUTPUT_PRIMITIVE TessOutPrim;
      TESSELLATOR_DOMAIN TessDomain;
      float MaxTessFactor;
