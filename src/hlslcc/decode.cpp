...
enum {
  FOURCC_PSG1 = FOURCC('P', 'S', 'G', '1')
}; // Patch constant signature with MinPrecision
...
// mark it as a shadow type. (e.g. accessed via sampler2DShadow rather than
// sampler2D)
static void MarkTextureAsShadow(ShaderInfo *psShaderInfo,
                                std::vector<Declaration> &declarations,
                                const Operand *psTextureOperand) {
  ASSERT(psTextureOperand->eType == OPERAND_TYPE_RESOURCE);

  for (std::vector<Declaration>::iterator psDecl = declarations.begin();
       psDecl != declarations.end(); psDecl++) {
    if (psDecl->eOpcode == OPCODE_DCL_RESOURCE) {
      if (psDecl->operands[0].eType == OPERAND_TYPE_RESOURCE &&
          psDecl->operands[0].ui32RegisterNumber ==
              psTextureOperand->ui32RegisterNumber) {
...
std::string TextureSamplerName(ShaderInfo *psShaderInfo,
  ...
  foundTexture = psShaderInfo->GetResourceFromBindingPoint(
      RGROUP_TEXTURE, ui32TextureRegisterNumber, &psTextureBinding);
  foundSampler = psShaderInfo->GetResourceFromBindingPoint(
      RGROUP_SAMPLER, ui32SamplerRegisterNumber, &psSamplerBinding);

  if (!foundTexture || !foundSampler) {
    oss << "UnknownResource" << ui32TextureRegisterNumber << "_"
        << ui32SamplerRegisterNumber;
    return oss.str();
  }

  ui32ArrayOffset = ui32TextureRegisterNumber - psTextureBinding->bindPoint;
  ...
  if (ui32ArrayOffset) {
    oss << texName << ui32ArrayOffset << "TEX_with_SMP"
        << psSamplerBinding->name;
  } else {
...
uint32_t DecodeOperand(const uint32_t *pui32Tokens, Operand *psOperand) {
  ...
  if (psOperand->iExtended) {
    ...
    if (DecodeExtendedOperandType(pui32Tokens[1]) ==
        EXTENDED_OPERAND_MODIFIER) {
      psOperand->eModifier = DecodeExtendedOperandModifier(pui32Tokens[1]);
      psOperand->eMinPrecision =
          (OPERAND_MIN_PRECISION)DecodeOperandMinPrecision(pui32Tokens[1]);
    }
  }
  ...
  if (psOperand->iWriteMaskEnabled && psOperand->iNumComponents == 4) {
    ...
    if (psOperand->eSelMode == OPERAND_4_COMPONENT_MASK_MODE) {
      ...
      if (psOperand->ui32Swizzle != NO_SWIZZLE) {
        psOperand->aui32Swizzle[0] =
            DecodeOperand4CompSwizzleSource(*pui32Tokens, 0);
        psOperand->aui32Swizzle[1] =
            DecodeOperand4CompSwizzleSource(*pui32Tokens, 1);
        psOperand->aui32Swizzle[2] =
            DecodeOperand4CompSwizzleSource(*pui32Tokens, 2);
        psOperand->aui32Swizzle[3] =
            DecodeOperand4CompSwizzleSource(*pui32Tokens, 3);
      } else {
  ...
  for (i = 0; i < psOperand->iIndexDims; ++i) {
    OPERAND_INDEX_REPRESENTATION eRep =
        DecodeOperandIndexRepresentation(i, *pui32Tokens);
    ...
    case OPERAND_INDEX_RELATIVE: {
      ...
      DecodeOperand(pui32Tokens + ui32NumTokens,
                    psOperand->m_SubOperands[i].get());
...
const uint32_t *DecodeDeclaration(Shader *psShader, const uint32_t *pui32Token,
                                  Declaration *psDecl, ShaderPhase *psPhase) {
  ...
  case OPCODE_DCL_RESOURCE: // DCL* opcodes have
  {
    ...
    DecodeOperand(pui32Token + ui32OperandOffset, &psDecl->operands[0]);
    break;
  }
  ...
  case OPCODE_DCL_INDEX_RANGE: {
    ...
    regSpace = psDecl->operands[0].GetRegisterSpace(psShader->eShaderType,
                                                    psPhase->ePhase);
    if (psDecl->operands[0].eType == OPERAND_TYPE_INPUT) {
      uint32_t i;
      const uint32_t indexRange = psDecl->value.ui32IndexRange;
      const uint32_t reg = psDecl->operands[0].ui32RegisterNumber;
    ...
    if (psDecl->operands[0].eType == OPERAND_TYPE_OUTPUT) {
      psShader
          ->aIndexedOutput[regSpace][psDecl->operands[0].ui32RegisterNumber] =
          true;
    }
    break;
  }
  case OPCODE_DCL_GS_OUTPUT_PRIMITIVE_TOPOLOGY: {
    psDecl->value.eOutputPrimitiveTopology =
        DecodeGSOutputPrimitiveTopology(*pui32Token);

    psShader->info.reflection.Geometry.outputTopology =
        psDecl->value.eOutputPrimitiveTopology;
    break;
  }
  case OPCODE_DCL_GS_INPUT_PRIMITIVE: {
    ...
    psShader->info.reflection.Geometry.inputPrimitive =
        psDecl->value.eInputPrimitive;
    break;
  }
  case OPCODE_DCL_MAX_OUTPUT_VERTEX_COUNT: {
    psDecl->value.ui32MaxOutputVertexCount = pui32Token[1];

    psShader->info.reflection.Geometry.maxVertexOutput =
  ...
  case OPCODE_DCL_TESS_PARTITIONING: {
    ...
    psShader->info.reflection.Tess.TessPartitioning =
  ...
  case OPCODE_DCL_TESS_DOMAIN: {
    ...
    psShader->info.reflection.Tess.TessDomain = psDecl->value.eTessDomain;
    break;
  }
  case OPCODE_DCL_TESS_OUTPUT_PRIMITIVE: {
    psDecl->value.eTessOutPrim = DecodeTessOutPrim(*pui32Token);

    psShader->info.reflection.Tess.TessOutPrim = psDecl->value.eTessOutPrim;
    break;
  }
  case OPCODE_DCL_THREAD_GROUP: {
    psShader->info.reflection.WorkGroupSize[0] =
        psDecl->value.aui32WorkGroupSize[0] = pui32Token[1];
    psShader->info.reflection.WorkGroupSize[1] =
        psDecl->value.aui32WorkGroupSize[1] = pui32Token[2];
    psShader->info.reflection.WorkGroupSize[2] =
  ...
  case OPCODE_DCL_INPUT_PS: {
    psDecl->ui32NumOperands = 1;
    psDecl->value.eInterpolation = DecodeInterpolationMode(*pui32Token);
    Operand *psOperand = &psDecl->operands[0];
  ...
  case OPCODE_DCL_INPUT_PS_SGV: {
    psDecl->ui32NumOperands = 1;
    DecodeOperand(pui32Token + ui32OperandOffset, &psDecl->operands[0]);
    DecodeNameToken(pui32Token + 3, &psDecl->operands[0]);
    break;
  }
  ...
  case OPCODE_DCL_INDEXABLE_TEMP: {
    ...
    psDecl->sIdxTemp.ui32RegComponentSize =
        *(pui32Token + ui32OperandOffset + 2);
    break;
  }
  ...
  case OPCODE_DCL_INTERFACE: {
    uint32_t func = 0, numClassesImplementingThisInterface, arrayLen,
             interfaceID;
    ...
    numClassesImplementingThisInterface =
        DecodeInterfaceTableLength(*(pui32Token + ui32OperandOffset));
    ...
    psShader->funcPointer[interfaceID].ui32NumBodiesPerTable =
        psDecl->ui32TableLength;
  ...
  case OPCODE_DCL_FUNCTION_TABLE: {
    ...
    for (ui32Func = 0; ui32Func < ui32NumFuncsInTable; ++ui32Func) {
      ...
      psShader->funcTable[ui32FuncTableID].aui32FuncBodies[ui32Func] =
          ui32FuncBodyID;
    }
  ...
  case OPCODE_DCL_INPUT_CONTROL_POINT_COUNT: {
    psShader->info.reflection.Domain.inputControlPoints =
        psDecl->value.ui32MaxOutputVertexCount =
            DecodeOutputControlPointCount(*pui32Token);
    break;
  }
  ...
  case OPCODE_DCL_OUTPUT_CONTROL_POINT_COUNT: {
    psShader->info.reflection.Tess.outputControlPoints =
        psDecl->value.ui32MaxOutputVertexCount =
            DecodeOutputControlPointCount(*pui32Token);
    break;
  }
  ...
  case OPCODE_CUSTOMDATA: {
    ui32TokenLength = pui32Token[1];
    {
      ...
      psDecl->asImmediateConstBuffer.assign(pVec4Array,
                                            pVec4Array + ui32NumVec4);
  ...
  case OPCODE_DCL_HS_MAX_TESSFACTOR: {
    psShader->info.reflection.Tess.MaxTessFactor =
  ...
  case OPCODE_DCL_UNORDERED_ACCESS_VIEW_TYPED: {
    ...
    psDecl->value.eResourceDimension = DecodeResourceDimension(*pui32Token);
    psDecl->sUAV.ui32GloballyCoherentAccess =
        DecodeAccessCoherencyFlags(*pui32Token);
    psDecl->sUAV.bCounter = 0;
    psDecl->ui32BufferStride = 4;
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psDecl->operands[0]);
    psDecl->sUAV.Type =
        DecodeResourceReturnType(0, pui32Token[ui32OperandOffset]);
    break;
  }
  ...
  case OPCODE_DCL_UNORDERED_ACCESS_VIEW_STRUCTURED: {
    ...
    psShader->info.GetResourceFromBindingPoint(
        RGROUP_UAV, psDecl->operands[0].ui32RegisterNumber, &psBinding);
    psShader->info.GetConstantBufferFromBindingPoint(
        RGROUP_UAV, psBinding->bindPoint, &psBuffer);
    psDecl->ui32BufferStride = psBuffer->ui32TotalSizeInBytes;

    switch (psBinding->resourceType) {
  ...
  case OPCODE_DCL_RESOURCE_STRUCTURED: {
    ...
    psShader->info.GetResourceFromBindingPoint(
        RGROUP_TEXTURE, psDecl->operands[0].ui32RegisterNumber, &psBinding);
    psShader->info.GetConstantBufferFromBindingPoint(
        RGROUP_TEXTURE, psBinding->bindPoint, &psBuffer);
    psDecl->ui32BufferStride = psBuffer->ui32TotalSizeInBytes;
    break;
  }
  ...
  case OPCODE_DCL_GS_INSTANCE_COUNT: {
    ...
    psShader->info.reflection.Geometry.instanceCount =
...
const uint32_t *DecodeInstruction(const uint32_t *pui32Token,
                                  Instruction *psInst, Shader *psShader,
                                  ShaderPhase *psPhase) {
  uint32_t ui32TokenLength = DecodeInstructionLength(*pui32Token);
  const uint32_t bExtended = DecodeIsOpcodeExtended(*pui32Token);
  const OPCODE_TYPE eOpcode = DecodeOpcodeType(*pui32Token);
  ...
  if (bExtended) {
    do {
      ...
      const EXTENDED_OPCODE_TYPE eExtType =
          DecodeExtendedOpcodeType(ui32ExtOpcodeToken);
      ...
      if (eExtType == EXTENDED_OPCODE_SAMPLE_CONTROLS) {
        ...
        sU.i4 = DecodeImmediateAddressOffset(IMMEDIATE_ADDRESS_OFFSET_U,
                                             ui32ExtOpcodeToken);
        sV.i4 = DecodeImmediateAddressOffset(IMMEDIATE_ADDRESS_OFFSET_V,
                                             ui32ExtOpcodeToken);
        sW.i4 = DecodeImmediateAddressOffset(IMMEDIATE_ADDRESS_OFFSET_W,
                                             ui32ExtOpcodeToken);
  ...
  case OPCODE_LABEL: {
    ...
    psInst->ui32FirstSrc = 0;
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[0]);
    break;
  }
  ...
  case OPCODE_MOV: {
    ...
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[0]);
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[1]);
    break;
  }
  case OPCODE_LOG:
  case OPCODE_RSQ:
  ...
  case OPCODE_DDIV: {
    ...
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[1]);
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[2]);
    break;
  }
  ...
  case OPCODE_IMUL: {
    ...
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[2]);
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[3]);
    break;
  }
  ...
  case OPCODE_IMM_ATOMIC_CMP_EXCH: {
    ...
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[3]);
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[4]);
    break;
  }
  ...
  case OPCODE_SAMPLE_B: {
    ...
    if (eOpcode != OPCODE_SAMPLE_B) {
      MarkTextureAsShadow(&psShader->info, psPhase->psDecl,
                          &psInst->operands[2]);
    }
  ...
  case OPCODE_SAMPLE_D: {
    ...
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[4]);
    ui32OperandOffset +=
        DecodeOperand(pui32Token + ui32OperandOffset, &psInst->operands[5]);
  ...
  // For opcodes that sample textures, mark which samplers are used by each
  // texture
  {
...
const uint32_t *DecodeShaderPhase(const uint32_t *pui32Tokens, Shader *psShader,
  ...
  while (1) // Keep going until we reach the first non-declaration token, or the
            // end of the shader.
  {
    psPhase->psDecl.push_back(Declaration());
    const uint32_t *pui32Result = DecodeDeclaration(
        psShader, pui32CurrentToken,
        &psPhase->psDecl[psPhase->psDecl.size() - 1], psPhase);
    ...
    if (pui32Result) {
      ...
      psPhase->psDecl
          .pop_back(); // Remove the last one, it wasn't needed after all
      break;
    }
  }
  ...
  while (pui32CurrentToken < (psShader->pui32FirstToken + ui32ShaderLength)) {
    ...
    const uint32_t *nextInstr = DecodeInstruction(
        pui32CurrentToken, &psPhase->psInst[psPhase->psInst.size() - 1],
        psShader, psPhase);
    ...
    if (psPhase->psInst[psPhase->psInst.size() - 1].eOpcode ==
            OPCODE_HS_FORK_PHASE ||
        psPhase->psInst[psPhase->psInst.size() - 1].eOpcode ==
...
const void AllocateHullPhaseArrays(const uint32_t *pui32Tokens,
  ...
  const uint32_t *pui32CurrentToken = pui32Tokens;
  const uint32_t ui32ShaderLength = psShader->ui32ShaderLength;
  uint32_t ui32PhaseCount =
      2; // Always the main phase and the HS global declarations
  uint32_t i;
  ...
  psShader->phases.clear();
  psShader->phases.resize(ui32PhaseCount);
  for (i = 0; i < ui32PhaseCount; i++)
    psShader->phases[i].ui32InstanceCount = 1;
}

const uint32_t *DecodeHullShader(const uint32_t *pui32Tokens,
                                 Shader *psShader) {
  const uint32_t *pui32CurrentToken = pui32Tokens;
  const uint32_t ui32ShaderLength = psShader->ui32ShaderLength;
  ShaderPhase *psPhase;
  ...
  psShader->phases[1].psInst.clear();
  psShader->phases[1].psDecl.clear();
  psShader->phases[1].ePhase = HS_GLOBAL_DECL_PHASE;
  psShader->phases[1].ui32InstanceCount = 1;

  // The next phase to parse in.
  psPhase = &psShader->phases[2];

  // Keep going until we have done all phases or the end of the shader.
  while (1) {
    Declaration newDecl;
    const uint32_t *pui32Result =
        DecodeDeclaration(psShader, pui32CurrentToken, &newDecl, psPhase);

    if (pui32Result) {
      pui32CurrentToken = pui32Result;

      if (newDecl.eOpcode == OPCODE_HS_CONTROL_POINT_PHASE) {
        pui32CurrentToken = DecodeShaderPhase(pui32CurrentToken, psShader,
                                              HS_CTRL_POINT_PHASE, psPhase);
        psPhase++;
      } else if (newDecl.eOpcode == OPCODE_HS_FORK_PHASE) {
        pui32CurrentToken = DecodeShaderPhase(pui32CurrentToken, psShader,
                                              HS_FORK_PHASE, psPhase++);
      } else if (newDecl.eOpcode == OPCODE_HS_JOIN_PHASE) {
        pui32CurrentToken = DecodeShaderPhase(pui32CurrentToken, psShader,
                                              HS_JOIN_PHASE, psPhase++);
      } else {
        psShader->phases[1].psDecl.push_back(newDecl);
      }
...
void Decode(const uint32_t *pui32Tokens, Shader *psShader) {
  ...
  if (psShader->eShaderType == HULL_SHADER) {
    ...
    psShader->phases.clear();
    psShader->phases.resize(1);
  }

  // Phase 0 is always the main phase
  psShader->phases[0].ui32InstanceCount = 1;
...
Shader *DecodeDXBC(uint32_t *data) {
  ...
  for (i = 0; i < chunkCount; ++i) {
    ...
    default: {
      //              ASSERT(0); //  Uncomment this to hunt for unknown chunks
      //              later on.
      break;
    }
    }
  ...
  if (shaderChunk) {
    ...
    LoadShaderInfo(ui32MajorVersion, ui32MinorVersion, &refChunks,
                   psShader->info);
