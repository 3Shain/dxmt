...
// Returns the result swizzle operand for an instruction, or NULL if all src
// operands have swizzles
static Operand *GetSrcSwizzleOperand(Instruction *psInst) {
  ...
  case OPCODE_RESINFO:
    return &psInst->operands[2];

  case OPCODE_GATHER4_PO:
  case OPCODE_GATHER4_PO_C:
  case OPCODE_LD_STRUCTURED:
    return &psInst->operands[3];

  case OPCODE_SAMPLE_INFO:
    return &psInst->operands[1];

  case OPCODE_ISHL:
  case OPCODE_ISHR:
  case OPCODE_USHR:
    // sm4 variant has single component selection on src1 -> only src0 has
    // swizzle
    if (psInst->operands[2].eSelMode == OPERAND_4_COMPONENT_SELECT_1_MODE)
      return &psInst->operands[1];
...
// Tweak the source operands of an instruction so that the rebased write mask
// will still work
static void DoSrcOperandRebase(Operand *psOperand, uint32_t rebase) {
  ...
  case OPERAND_4_COMPONENT_MASK_MODE:
    ASSERT(psOperand->ui32CompMask == 0 ||
           psOperand->ui32CompMask == OPERAND_4_COMPONENT_MASK_ALL);
    ...
    if (psOperand->eType == OPERAND_TYPE_IMMEDIATE32) {
      if (psOperand->iNumComponents > 1)
        std::copy(&psOperand->afImmediates[rebase], &psOperand->afImmediates[4],
                  &psOperand->afImmediates[0]);
      return;
    }
    if (psOperand->eType == OPERAND_TYPE_IMMEDIATE64) {
      if (psOperand->iNumComponents > 1)
        std::copy(&psOperand->adImmediates[rebase], &psOperand->adImmediates[4],
...
void Instruction::ChangeOperandTempRegister(Operand *psOperand, uint32_t oldReg,
  ...
  if (flags & UD_CHANGE_SUBOPERANDS) {
    for (i = 0; i < MAX_SUB_OPERANDS; i++) {
      if (psOperand->m_SubOperands[i].get())
        ChangeOperandTempRegister(psOperand->m_SubOperands[i].get(), oldReg,
                                  newReg, compMask, UD_CHANGE_ALL, rebase);
    }
  }
  ...
  // If this operation touches other components than the one(s) we're splitting,
  // skip it
  if ((accessMask & (~compMask)) != 0) {
    ...
    // a component in multi-component mode and we're supposed to treat it as
    // single-use only. Now that we track operands we can bring this back
  ...
  // Update component mask. Note that we don't need to do anything to the
  // suboperands. They do not affect destination writemask.
  switch (psOperand->eSelMode) {
  case OPERAND_4_COMPONENT_MASK_MODE: {
  ...
  case OPERAND_4_COMPONENT_SWIZZLE_MODE: {
    for (i = 0; i < 4; i++) {
      // Note that this rebase is different from the one done for source
      // operands
  ...
  // Nasty corner case of 2 destinations, not supported if both targets are
  // written
  ASSERT((ui32FirstSrc < 2) || (operands[0].eType == OPERAND_TYPE_NULL) ||
         (operands[1].eType == OPERAND_TYPE_NULL));

  // If we made it this far, we're rebasing a destination temp (and the only
  // destination), need to tweak sources depending on the instruction
  switch (eOpcode) {
  ...
  default:
    psSwizzleOperand =
        GetSrcSwizzleOperand(this); // Null means tweak all source operands
    if (psSwizzleOperand) {
      DoSrcOperandRebase(psSwizzleOperand, rebase);
      return;
    } else {
      for (i = ui32FirstSrc; i < ui32NumOperands; i++) {
        DoSrcOperandRebase(&operands[i], rebase);
      }
    }
    return;
  }
