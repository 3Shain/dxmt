...
// Returns nonzero if types are just different precisions of the same underlying
// type
static bool AreTypesCompatibleMetal(SHADER_VARIABLE_TYPE a,
                                    uint32_t ui32TOFlag) {
...
std::string ToMetal::TranslateOperandSwizzle(const Operand *psOperand,
                                             uint32_t ui32ComponentMask,
                                             int iRebase,
  ...
  if (psOperand->eType == OPERAND_TYPE_INPUT) {
    ...
    if (regSpace == 0) {
      if ((psContext->psShader->phases[psContext->currentPhase]
               .acInputNeedsRedirect[psOperand->ui32RegisterNumber] == 0) &&
          (psContext->psShader
               ->abScalarInput[regSpace][psOperand->ui32RegisterNumber] &
           accessMask)) {
        return "";
      }
    } else {
      if ((psContext->psShader->phases[psContext->currentPhase]
               .acPatchConstantsNeedsRedirect[psOperand->ui32RegisterNumber] ==
           0) &&
          (psContext->psShader
               ->abScalarInput[regSpace][psOperand->ui32RegisterNumber] &
           accessMask)) {
  ...
  if (psOperand->eType == OPERAND_TYPE_OUTPUT) {
    ...
    if (regSpace == 0) {
      if ((psContext->psShader->phases[psContext->currentPhase]
               .acOutputNeedsRedirect[psOperand->ui32RegisterNumber] == 0) &&
          (psContext->psShader
               ->abScalarOutput[regSpace][psOperand->ui32RegisterNumber] &
           accessMask)) {
        return "";
      }
    } else {
  ...
  if (psOperand->iWriteMaskEnabled && psOperand->iNumComponents != 1) {
    ...
    if (psOperand->eSelMode == OPERAND_4_COMPONENT_MASK_MODE) {
      ...
      } else if (psOperand->eSelMode ==
                 OPERAND_4_COMPONENT_SELECT_1_MODE) // ui32ComponentMask is
...
std::string ToMetal::TranslateOperandIndex(const Operand *psOperand,
                                           int index) {
  int i = index;
  std::ostringstream oss;
...
SHADER_VARIABLE_TYPE from, SHADER_VARIABLE_TYPE to, uint32_t numComponents)
{
    ...
    {
        if ((to == SVT_FLOAT || to == SVT_FLOAT16 || to == SVT_FLOAT10) && from
== SVT_INT) return "intBitsToFloat"; else if ((to == SVT_FLOAT || to ==
SVT_FLOAT16 || to == SVT_FLOAT10) && from == SVT_UINT) return "uintBitsToFloat";
...
// Helper function to print out a single 32-bit immediate value in desired
// format
static std::string printImmediate32(uint32_t value,
                                    SHADER_VARIABLE_TYPE eType) {
  std::ostringstream oss;
  int needsParenthesis = 0;

  // Print floats as bit patterns.
  if ((eType == SVT_FLOAT || eType == SVT_FLOAT16 || eType == SVT_FLOAT10) &&
      fpcheck(*((float *)(&value)))) {
    oss << "as_type<float>(";
    eType = SVT_INT;
    needsParenthesis = 1;
...
static std::string MakeCBVarName(const std::string &cbName,
                                 const std::string &fullName) {
  return cbName + fullName;
}

std::string ToMetal::TranslateVariableName(const Operand *psOperand,
                                           uint32_t ui32TOFlag,
  ...
  *pui32IgnoreSwizzle = 0;
  ...
  if (psOperand->eType == OPERAND_TYPE_TEMP) {
    // Check for scalar
    if (psContext->psShader->GetTempComponentCount(
            eType, psOperand->ui32RegisterNumber) == 1 &&
  ...
  if (psOperand->eType == OPERAND_TYPE_INPUT) {
    ...
    //   checking abScalarInput might report as scalar, while in reality that
    //   was redirected and now is vector so swizzle must be preserved as an
    //   example consider we have input:
    //     float2 x; float y;
    //   and later on we do
    //     tex2D(xxx, fixed2(x.x, y));
    //   in that case we will generate redirect but which ui32RegisterNumber
    //   will be used for it is not strictly "specified" so we may end up with
    //   treating it as scalar (even though it is vector now)
    const int redirectInput =
    ...
    const int scalarInput =
        psContext->psShader->abScalarInput[psOperand->GetRegisterSpace(
            psContext)][psOperand->ui32RegisterNumber];
    if (!wasRedirected && (scalarInput & psOperand->GetAccessMask()) &&
        (psOperand->eSelMode == OPERAND_4_COMPONENT_SWIZZLE_MODE)) {
      scalarWithSwizzle = 1;
  ...
  if (!(ui32TOFlag &
        (TO_FLAG_DESTINATION | TO_FLAG_NAME_ONLY | TO_FLAG_DECLARATION_NAME))) {
    ...
    if (AreTypesCompatibleMetal(eType, ui32TOFlag) == 0) {
      if (CanDoDirectCast(psContext, eType, requestedType)) {
        ...
        oss << GetConstructorForType(psContext, requestedType,
                                     requestedComponents)
            << "(";
        numParenthesis++;
      } else {
        ...
        oss << "as_type<"
            << GetConstructorForTypeMetal(requestedType, requestedComponents)
            << ">(";
        hasCtor = 1;
        bitcast = true;
        numParenthesis++;
    ...
    // Add ctor if needed (upscaling). Type conversion is already handled above,
    // so here we must use the original type to not make type conflicts in
    // bitcasts
    bool needsUpscaling =
        ((numComponents < requestedComponents) || (scalarWithSwizzle != 0)) &&
        (hasCtor == 0 || bitcast);

    // Add constuctor if half precision is forced to avoid template ambiguity
    // error from compiler
    bool needsForcedCtor = (ui32TOFlag & TO_FLAG_FORCE_HALF) &&
                           (psOperand->eType == OPERAND_TYPE_IMMEDIATE32 ||
                            psOperand->eType == OPERAND_TYPE_IMMEDIATE64);
    ...
    if (needsUpscaling || needsForcedCtor) {
      oss << GetConstructorForType(psContext, eType, requestedComponents)
          << "(";
  ...
  case OPERAND_TYPE_IMMEDIATE32: {
    if (psOperand->iNumComponents == 1) {
      oss << printImmediate32(*((unsigned int *)(&psOperand->afImmediates[0])),
                              requestedType);
    } else {
      int i;
      ...
      for (i = 0; i < 4; i++) {
        ...
        uval = *(
            (uint32_t *)(&psOperand
                              ->afImmediates[i >= psOperand->iNumComponents
  ...
  case OPERAND_TYPE_INPUT: {
    ...
    case INDEX_2D: {
      ...
      if (psContext->psShader->eShaderType == HULL_SHADER ||
        ...
        oss << "."
            << psContext->GetDeclaredInputName(psOperand, piRebase, 1,
                                               pui32IgnoreSwizzle);
      } else {
        // Not sure if this codepath is active outside hull/domain
        oss << psContext->GetDeclaredInputName(psOperand, piRebase, 0,
                                               pui32IgnoreSwizzle);
    ...
    default: {
      if (psOperand->eIndexRep[0] == OPERAND_INDEX_IMMEDIATE32_PLUS_RELATIVE) {
        ASSERT(psContext->psShader
                   ->aIndexedInput[regSpace][psOperand->ui32RegisterNumber] !=
               0);
        oss << "phase" << psContext->currentPhase << "_Input" << regSpace << "_"
            << psOperand->ui32RegisterNumber << "[";
        oss << TranslateOperand(psOperand->m_SubOperands[0].get(),
                                TO_FLAG_INTEGER);
        oss << "]";
      } else {
        if (psContext->psShader
                ->aIndexedInput[regSpace][psOperand->ui32RegisterNumber] != 0) {
          const uint32_t parentIndex =
              psContext->psShader
                  ->aIndexedInputParents[regSpace]
                                        [psOperand->ui32RegisterNumber];
          oss << "phase" << psContext->currentPhase << "_Input" << regSpace
              << "_" << parentIndex << "["
  ...
  case OPERAND_TYPE_OUTPUT_DEPTH_LESS_EQUAL: {
    int stream = 0;
    oss << psContext->GetDeclaredOutputName(psOperand, &stream,
                                            pui32IgnoreSwizzle, piRebase, 0);
  ...
  case OPERAND_TYPE_TEMP: {
    ...
    ASSERT(psOperand->ui32RegisterNumber <
           0x10000); // Sanity check after temp splitting.
    switch (eTempType) {
    case SVT_FLOAT:
      ASSERT(psContext->psShader
                 ->psFloatTempSizes[psOperand->ui32RegisterNumber] != 0);
      if (psContext->psShader
              ->psFloatTempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_FLOAT16:
      ASSERT(psContext->psShader
                 ->psFloat16TempSizes[psOperand->ui32RegisterNumber] != 0);
      oss << ("16_");
      if (psContext->psShader
              ->psFloat16TempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_FLOAT10:
      ASSERT(psContext->psShader
                 ->psFloat10TempSizes[psOperand->ui32RegisterNumber] != 0);
      oss << ("10_");
      if (psContext->psShader
              ->psFloat10TempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_INT:
      ASSERT(
          psContext->psShader->psIntTempSizes[psOperand->ui32RegisterNumber] !=
          0);
      oss << ("i");
      if (psContext->psShader->psIntTempSizes[psOperand->ui32RegisterNumber] ==
          1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_INT16:
      ASSERT(psContext->psShader
                 ->psInt16TempSizes[psOperand->ui32RegisterNumber] != 0);
      oss << ("i16_");
      if (psContext->psShader
              ->psInt16TempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_INT12:
      ASSERT(psContext->psShader
                 ->psInt12TempSizes[psOperand->ui32RegisterNumber] != 0);
      oss << ("i12_");
      if (psContext->psShader
              ->psInt12TempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_UINT:
      ASSERT(
          psContext->psShader->psUIntTempSizes[psOperand->ui32RegisterNumber] !=
          0);
      oss << ("u");
      if (psContext->psShader->psUIntTempSizes[psOperand->ui32RegisterNumber] ==
          1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_UINT16:
      ASSERT(psContext->psShader
                 ->psUInt16TempSizes[psOperand->ui32RegisterNumber] != 0);
      oss << ("u16_");
      if (psContext->psShader
              ->psUInt16TempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_DOUBLE:
      ASSERT(psContext->psShader
                 ->psDoubleTempSizes[psOperand->ui32RegisterNumber] != 0);
      oss << ("d");
      if (psContext->psShader
              ->psDoubleTempSizes[psOperand->ui32RegisterNumber] == 1)
        *pui32IgnoreSwizzle = 1;
      break;
    case SVT_BOOL:
      ASSERT(
          psContext->psShader->psBoolTempSizes[psOperand->ui32RegisterNumber] !=
          0);
      oss << ("b");
      if (psContext->psShader->psBoolTempSizes[psOperand->ui32RegisterNumber] ==
          1)
        *pui32IgnoreSwizzle = 1;
      break;
    default:
  ...
  case OPERAND_TYPE_CONSTANT_BUFFER: {
    const ConstantBuffer *constantBuffer = NULL;
    ...
    psContext->psShader->info.GetConstantBufferFromBindingPoint(
        RGROUP_CBUFFER, psOperand->aui32ArraySizes[0], &constantBuffer);
    ...
    if (constantBuffer) {
      //$Globals.
      cbName = "arguments." + GetCBName(constantBuffer->name);
      cbName += ".";
    }
    ...
    if ((ui32TOFlag & TO_FLAG_DECLARATION_NAME) != TO_FLAG_DECLARATION_NAME) {
      ...
      // When we have a component mask that doesn't have .x set (this basically
      // only happens when we manually open operands into components) We have to
      // pull down the swizzle array to match the first bit that's actually set
      ...
      while ((ui32CompMask & (1 << firstBitSet)) == 0)
        firstBitSet++;
      std::copy(&psOperand->aui32Swizzle[firstBitSet],
                &psOperand->aui32Swizzle[4], &tmpSwizzle[0]);

      ShaderInfo::GetShaderVarFromOffset(
          psOperand->aui32ArraySizes[1], tmpSwizzle, constantBuffer, &psVarType,
          &isArray, &arrayIndices, &rebase, psContext->flags);

      // Get a possible dynamic array index
      std::string dynamicIndexStr;
      bool needsIndexCalcRevert = false;
      bool isAoS = ((!isArray && arrayIndices.size() > 0) ||
                    (isArray && arrayIndices.size() > 1));
      Operand *psDynIndexOp = psOperand->GetDynamicIndexOperand(
          psContext, psVarType, isAoS, &needsIndexCalcRevert);
      ...
      if (psDynIndexOp != NULL) {
        ...
        dynamicIndexStr = TranslateOperand(
            psDynIndexOp, opFlags,
            0x1); // Just take the first component for the index
      }

      if (psOperand->eSelMode == OPERAND_4_COMPONENT_SELECT_1_MODE ||
          (componentsNeeded <= psVarType->Columns)) {
        // Simple case: just access one component
        std::string fullName = ShaderInfo::GetShaderVarIndexedFullName(
            psVarType, arrayIndices, dynamicIndexStr, needsIndexCalcRevert,
            psContext->flags & HLSLCC_FLAG_TRANSLATE_MATRICES);

        // Special hack for MSAA subpass inputs: in Metal we can only read the
        // "current" sample, so ignore the index
        if (strncmp(fullName.c_str(), "hlslcc_fbinput", 14) == 0)
          isFBInput = true;

        if (((psContext->flags & HLSLCC_FLAG_TRANSLATE_MATRICES) != 0) &&
            ((psVarType->Class == SVC_MATRIX_ROWS) ||
          ...
          char prefix[256];
          sprintf(prefix, HLSLCC_TRANSLATE_MATRIX_FORMAT_STRING,
                  psVarType->Rows, psVarType->Columns);
        ...
        oss << MakeCBVarName(cbName, fullName);
      } else {
        ...
        oss << GetConstructorForTypeMetal(psVarType->Type,
                                          GetNumberBitsSet(ui32CompMask))
            << "(";
        for (i = 0; i < 4; i++) {
          ...
          uint32_t tmpSwizzle[4] = {0};
          std::copy(&psOperand->aui32Swizzle[i], &psOperand->aui32Swizzle[4],
                    &tmpSwizzle[0]);

          ShaderInfo::GetShaderVarFromOffset(
              psOperand->aui32ArraySizes[1], tmpSwizzle, constantBuffer,
              &tmpVarType, &tmpIsArray, &tmpArrayIndices, &tmpRebase,
              psContext->flags);
          std::string fullName = ShaderInfo::GetShaderVarIndexedFullName(
              tmpVarType, tmpArrayIndices, dynamicIndexStr,
              needsIndexCalcRevert,
              psContext->flags & HLSLCC_FLAG_TRANSLATE_MATRICES);
          oss << MakeCBVarName(cbName, fullName);
      ...
      if (isArray) {
        ...
        // Dynamic index is atm supported only at the root array level. Add here
        // only if there is no such parent.
        bool hasDynamicIndex =
            !dynamicIndexStr.empty() && (arrayIndices.size() <= 1);
        bool hasImmediateIndex =
            (index != -1) && !(hasDynamicIndex && index == 0);
        ...
        if (isFBInput) {
          ...
          if (((psVarType->Class == SVC_MATRIX_COLUMNS) ||
               (psVarType->Class == SVC_MATRIX_ROWS)) &&
              (psVarType->Elements > 1) &&
              ((psContext->flags & HLSLCC_FLAG_TRANSLATE_MATRICES) == 0)) {
      ...
      if (psVarType->Class == SVC_SCALAR) {
        *pui32IgnoreSwizzle = 1;
        ...
        // CB arrays are all declared as 4-component vectors to match DX11 data
        // layout. Therefore add swizzle here to access the element
        // corresponding to the scalar var.
        if ((psVarType->Elements > 0) &&
  ...
  case OPERAND_TYPE_RESOURCE: {
    oss << "arguments."
        << ResourceName(RGROUP_TEXTURE, psOperand->ui32RegisterNumber);
    *pui32IgnoreSwizzle = 1;
    break;
  }
  ...
  case OPERAND_TYPE_INPUT_JOIN_INSTANCE_ID: {
    oss << "phaseInstanceID"; // Not a real builtin, but passed as a function
                              // parameter.
    *pui32IgnoreSwizzle = 1;
    break;
  }
  ...
  case OPERAND_TYPE_INPUT_CONTROL_POINT: {
    ...
    if (ignoreRedirect) {
      ...
      oss << TranslateOperandIndex(psOperand, 0); // Vertex index
      oss << "."
          << psContext->GetDeclaredInputName(
                 psOperand, piRebase, ignoreRedirect, pui32IgnoreSwizzle);
    } else {
      oss << psContext->GetDeclaredInputName(
          psOperand, piRebase, ignoreRedirect, pui32IgnoreSwizzle);
      oss << TranslateOperandIndex(psOperand, 0); // Vertex index
    }

    // Check for scalar
    if ((psContext->psShader->abScalarInput[psOperand->GetRegisterSpace(
             psContext)][psOperand->ui32RegisterNumber] &
         psOperand->GetAccessMask()) != 0)
      *pui32IgnoreSwizzle = 1;
    break;
  }
  ...
  case OPERAND_TYPE_INDEXABLE_TEMP: {
    ...
    if (psOperand->aui32ArraySizes[1] != 0 ||
        !psOperand->m_SubOperands[1].get())
      oss << psOperand->aui32ArraySizes[1];

    if (psOperand->m_SubOperands[1].get()) {
      if (psOperand->aui32ArraySizes[1] != 0)
        oss << "+";
      oss << TranslateOperand(psOperand->m_SubOperands[1].get(),
                              TO_FLAG_INTEGER);
    }
    oss << "]";
    break;
  ...
  case OPERAND_TYPE_INPUT_PATCH_CONSTANT: {
    ...
    psContext->psShader->info.GetPatchConstantSignatureFromRegister(
        psOperand->ui32RegisterNumber, psOperand->GetAccessMask(), &psIn);
    *piRebase = psIn->iRebase;
    switch (psIn->systemValueType) {
    case NAME_POSITION:
      oss << "mtl_Position";
      break;
    ...
    // as far as i understand tesselation factors are always coming from
    // tessFactor variable (it is always declared in ToMetal::Translate)
    ...
    default:
      ...
      if (psContext->psShader->eShaderType == DOMAIN_SHADER)
        oss << psContext->inputPrefix << patchPrefix << psIn->semanticName
            << psIn->semanticIndex;
      else
        oss << patchPrefix << psIn->semanticName << psIn->semanticIndex;

      // Disable swizzles if this is a scalar
      if (psContext->psShader->eShaderType == HULL_SHADER) {
        if ((psContext->psShader
                 ->abScalarOutput[1][psOperand->ui32RegisterNumber] &
             psOperand->GetAccessMask()) != 0)
          *pui32IgnoreSwizzle = 1;
      } else {
        if ((psContext->psShader
                 ->abScalarInput[1][psOperand->ui32RegisterNumber] &
             psOperand->GetAccessMask()) != 0)
          *pui32IgnoreSwizzle = 1;
      }
      break;
  ...
  if (hasCtor && (*pui32IgnoreSwizzle == 0)) {
    oss << TranslateOperandSwizzle(psOperand, ui32CompMask,
                                   piRebase ? *piRebase : 0);
    *pui32IgnoreSwizzle = 1;
  }

  if (needsBoolUpscale) {
    if (requestedType == SVT_UINT || requestedType == SVT_UINT16 ||
        requestedType == SVT_UINT8)
      oss << ") * 0xffffffffu";
    else
      oss << ") * int(0xffffffffu)";
    numParenthesis--;
...
std::string ToMetal::TranslateOperand(const Operand *psOperand,
                                      uint32_t ui32TOFlag,
                                      uint32_t ui32ComponentMask) {
  std::ostringstream oss;
  ...
  if (ui32TOFlag & TO_FLAG_NAME_ONLY) {
    return TranslateVariableName(psOperand, ui32TOFlag, &ui32IgnoreSwizzle,
                                 OPERAND_4_COMPONENT_MASK_ALL, &iRebase);
  }
  ...
  oss << TranslateVariableName(psOperand, ui32TOFlag, &ui32IgnoreSwizzle,
                               ui32ComponentMask, &iRebase);
