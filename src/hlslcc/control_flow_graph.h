...
public:
  ControlFlowGraph() : m_BlockMap(), m_BlockStorage() {}

  typedef std::vector<std::shared_ptr<BasicBlock>> BasicBlockStorage;
  ...
  const BasicBlock *
  GetBasicBlockForInstruction(const Instruction *instruction) const;
...
private:
  // Map for storing the created basic blocks. Map key is the pointer to the
  // first instruction in the block
  typedef std::map<const Instruction *, BasicBlock *> BasicBlockMap;

  BasicBlockMap m_BlockMap;

  // auto_ptr -type storage for multiple BasicBlocks. BlockMap above only has
  // pointers into these
...
public:
  ...
  // The connections (either incoming or outgoing) from this block. The
  // instruction is the same one as the key in ControlFlowGraph to that basic
  // block
  typedef std::set<const Instruction *> ConnectionSet;

  struct Definition {
    Definition(const Instruction *i = nullptr, const Operand *o = nullptr)
        : m_Instruction(i), m_Operand(o) {}
  ...
  typedef std::set<Definition>
      ReachableDefinitionsPerVariable; // A set of possibly visible definitions
                                       // for one component of one vec4 variable
  typedef std::map<uint32_t, ReachableDefinitionsPerVariable>
      ReachableVariables; // A VisibleDefinitionSet for each variable*component.
...
private:
  ...
  BasicBlock(const Instruction *psFirst, ControlFlowGraph &graph,
             const Instruction *psPrecedingBlockHead, const Instruction *psEnd);

  // Walk through the instructions and build UEVar and VarKill sets, create
  // succeeding nodes if they don't exist already.
  void Build();

  bool RebuildReachable(); // Rebuild m_Reachable from preceding blocks and this
                           // one. Returns true if current value changed.
...
private:
  ...
  const Instruction *m_Last;  // The last instruction in the basic block. Either
                              // OPCODE_RET or a branch/jump/loop instruction
  const Instruction *m_End;   // past-the-end pointer

  RegisterSet m_UEVar; // Upwards-exposed variables (temps that need definition
                       // from upstream and are used in this basic block)
  RegisterSet m_VarKill; // Set of variables that are defined in this block.

  ConnectionSet m_Preceding;  // Set of blocks that immediately precede this
                              // block in the CFG
  ConnectionSet m_Succeeding; // Set of blocks that follow this block in the CFG

  ReachableVariables m_DEDef; // Downward-exposed definitions from this basic
                              // block. Always only one item per set.

  ReachableVariables m_Reachable; // The set of variable definitions that are
                                  // visible at the end of this block.
};
} // namespace ControlFlow
} // namespace dxmt
