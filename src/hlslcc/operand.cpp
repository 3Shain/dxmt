...
uint32_t Operand::GetAccessMask() const {
  ...
  // NOTE: Destination writemask can (AND DOES) affect access from sources, but
  // we do it conservatively for now.
  switch (eSelMode) {
  default:
...
int Operand::GetMaxComponent() const {
  if (iWriteMaskEnabled && iNumComponents == 4) {
    // Component Mask
    if (eSelMode == OPERAND_4_COMPONENT_MASK_MODE) {
      if (ui32CompMask != 0 &&
          ui32CompMask !=
              (OPERAND_4_COMPONENT_MASK_X | OPERAND_4_COMPONENT_MASK_Y |
               OPERAND_4_COMPONENT_MASK_Z | OPERAND_4_COMPONENT_MASK_W)) {
...
bool Operand::IsSwizzleReplicated() const {
  if (iWriteMaskEnabled && iNumComponents == 4) {
    if (eSelMode == OPERAND_4_COMPONENT_SWIZZLE_MODE) {
      if (ui32Swizzle == WWWW_SWIZZLE || ui32Swizzle == ZZZZ_SWIZZLE ||
          ui32Swizzle == YYYY_SWIZZLE || ui32Swizzle == XXXX_SWIZZLE) {
        return true;
      }
    }
  }
...
// Get the number of elements returned by operand, taking additional component
// mask into account
uint32_t Operand::GetNumSwizzleElements(
    uint32_t _ui32CompMask /* = OPERAND_4_COMPONENT_MASK_ALL */) const {
  uint32_t count = 0;
  ...
  case OPERAND_TYPE_OUTPUT_DEPTH: {
    ...
    return dxmt::GetNumberBitsSet(compMask);
  }
  default: {
    break;
  }
  ...
  if (iWriteMaskEnabled && iNumComponents != 1) {
    // Component Mask
    if (eSelMode == OPERAND_4_COMPONENT_MASK_MODE) {
      ...
      if (eSelMode == OPERAND_4_COMPONENT_SWIZZLE_MODE) {
        ...
        if (aui32Swizzle[0] == OPERAND_4_COMPONENT_X &&
            (_ui32CompMask & OPERAND_4_COMPONENT_MASK_X)) {
          count++;
        } else if (aui32Swizzle[0] == OPERAND_4_COMPONENT_Y &&
                   (_ui32CompMask & OPERAND_4_COMPONENT_MASK_Y)) {
          count++;
        } else if (aui32Swizzle[0] == OPERAND_4_COMPONENT_Z &&
                   (_ui32CompMask & OPERAND_4_COMPONENT_MASK_Z)) {
          count++;
        } else if (aui32Swizzle[0] == OPERAND_4_COMPONENT_W &&
...
// Returns 0 if the register used by the operand is per-vertex, or 1 if
// per-patch
int Operand::GetRegisterSpace(SHADER_TYPE eShaderType,
                              SHADER_PHASE_TYPE eShaderPhaseType) const {
  ...
  if (eType == OPERAND_TYPE_INPUT_CONTROL_POINT ||
      eType == OPERAND_TYPE_OUTPUT_CONTROL_POINT)
    return 0;
...
int Operand::GetRegisterSpace(const HLSLCrossCompilerContext *psContext) const {
  return GetRegisterSpace(
      psContext->psShader->eShaderType,
      psContext->psShader->phases[psContext->currentPhase].ePhase);
}

SHADER_VARIABLE_TYPE Operand::GetDataType(
    HLSLCrossCompilerContext *psContext,
    SHADER_VARIABLE_TYPE ePreferredTypeForImmediates /* = SVT_INT */) const {
  ...
  case OPERAND_TYPE_OUTPUT: {
    ...
    if (regSpace == 0)
      psContext->psShader->info.GetOutputSignatureFromRegister(
          ...
          psContext->psShader->ui32CurrentVertexOutputStream, &psOut);
    else {
      psContext->psShader->info.GetPatchConstantSignatureFromRegister(
          ui32Register, GetAccessMask(), &psOut, true);
      if (!psOut)
        return SVT_FLOAT;
    }

    ASSERT(psOut != NULL);
    if (psOut->minPrecision != MIN_PRECISION_DEFAULT) {
      switch (psOut->minPrecision) {
      default:
        ASSERT(0);
        break;
  ...
  case OPERAND_TYPE_INPUT_CONTROL_POINT: {
    ...
    if (regSpace == 0) {
      if (psContext->psShader->phases[psContext->currentPhase]
              .acInputNeedsRedirect[ui32Register] != 0)
        return SVT_FLOAT; // All combined inputs are stored as floats
      psContext->psShader->info.GetInputSignatureFromRegister(
          ui32Register, GetAccessMask(), &psIn);
    } else {
      if (psContext->psShader->phases[psContext->currentPhase]
              .acPatchConstantsNeedsRedirect[ui32Register] != 0)
        return SVT_FLOAT; // All combined inputs are stored as floats
      psContext->psShader->info.GetPatchConstantSignatureFromRegister(
          ui32Register, GetAccessMask(), &psIn);
    }
    ...
    if (psIn->systemValueType == NAME_IS_FRONT_FACE)
      return SVT_UINT;

    // UINT in DX, INT in GL.
    if (psIn->systemValueType == NAME_PRIMITIVE_ID ||
        psIn->systemValueType == NAME_VERTEX_ID ||
        psIn->systemValueType == NAME_INSTANCE_ID ||
        psIn->systemValueType == NAME_RENDER_TARGET_ARRAY_INDEX ||
        psIn->systemValueType == NAME_VIEWPORT_ARRAY_INDEX ||
        psIn->systemValueType == NAME_SAMPLE_INDEX)
      return SVT_UINT;

    if (psIn->minPrecision != MIN_PRECISION_DEFAULT) {
      switch (psIn->minPrecision) {
      default:
        ASSERT(0);
        break;
      case MIN_PRECISION_FLOAT_16:
  ...
  case OPERAND_TYPE_CONSTANT_BUFFER: {
    ...
    psContext->psShader->info.GetConstantBufferFromBindingPoint(
        RGROUP_CBUFFER, aui32ArraySizes[0], &psCBuf);
    if (psCBuf) {
      int foundVar = ShaderInfo::GetShaderVarFromOffset(
          aui32ArraySizes[1], aui32Swizzle, psCBuf, &psVarType, &isArray, NULL,
          &rebase, psContext->flags);
      if (foundVar)
  ...
  case OPERAND_TYPE_IMMEDIATE_CONSTANT_BUFFER: // constant array is floats
                                               // everywhere except on vulkan
  {
    return SVT_FLOAT;
  }
...
int Operand::GetNumInputElements(
    const HLSLCrossCompilerContext *psContext) const {
  ...
  return dxmt::GetNumberBitsSet(psSig->ui32Mask);
}

Operand *Operand::GetDynamicIndexOperand(HLSLCrossCompilerContext *psContext,
                                         const ShaderVarType *psVar, bool isAoS,
                                         bool *needsIndexCalcRevert) const {
  ...
  *needsIndexCalcRevert = false;
  if (psDynIndexOp != NULL && isAoS) {
    // if dynamically indexing array of structs, try using the original index
    // var before the float4 address calc
    bool indexVarFound = false;
    *needsIndexCalcRevert = true;
    Instruction *psDynIndexOrigin = psDynIndexOp->m_Defines[0].m_Inst;
    Operand *asOps = psDynIndexOrigin->operands;
    ...
    if (ShaderInfo::GetCBVarSize(psVar->Parent, true) <=
        16) // matrixAsVectors arg does not matter here as with matrices the
            // size will go over the limit anyway
    {
      indexVarFound = true;
      *needsIndexCalcRevert = false;
    } else if (psDynIndexOrigin->eOpcode == OPCODE_IMUL) {
      // check which one of the src operands is the original index
      if ((asOps[2].eType == OPERAND_TYPE_TEMP ||
           asOps[2].eType == OPERAND_TYPE_INPUT ||
           asOps[2].eType == OPERAND_TYPE_CONSTANT_BUFFER) &&
          asOps[3].eType == OPERAND_TYPE_IMMEDIATE32)
        psOriginOp = &asOps[2];
      else if ((asOps[3].eType == OPERAND_TYPE_TEMP ||
                asOps[3].eType == OPERAND_TYPE_INPUT ||
                asOps[3].eType == OPERAND_TYPE_CONSTANT_BUFFER) &&
               asOps[2].eType == OPERAND_TYPE_IMMEDIATE32)
        psOriginOp = &asOps[3];
    } else if (psDynIndexOrigin->eOpcode == OPCODE_ISHL) {
      if (asOps[2].eType == OPERAND_TYPE_IMMEDIATE32 &&
          asOps[1].eSelMode == OPERAND_4_COMPONENT_SWIZZLE_MODE)
        psOriginOp = &asOps[0];
    ...
    if (psOriginOp != NULL) {
      ...
      // Check if the mul dest is not the same temp as the src. Also check that
      // the temp does not have multiple uses (which could override the value)
      // -> we can use src straight and no index revert calc is needed
      if ((psOriginOp->eType == OPERAND_TYPE_INPUT) ||
          ((psOriginOp->ui32RegisterNumber !=
                psDynIndexOp->ui32RegisterNumber ||
            psOriginOp->GetDataType(psContext) !=
                psDynIndexOp->GetDataType(psContext)) &&
           (!psOriginOp->m_Defines.empty()) &&
    ...
    // Atm we support only this very basic case of dynamic indexing array of
    // structs. Return error if something else is encountered.
    if (!indexVarFound)
      psContext->m_Reflection.OnDiagnostics(
          "Unsupported dynamic indexing scheme on constant buffer vars.", 0,
          true);
  }
