...
inline static SHADER_TYPE DecodeShaderType(uint32_t ui32Token) {
  return (SHADER_TYPE)((ui32Token & 0xffff0000) >> 16);
}

inline static uint32_t DecodeProgramMajorVersion(uint32_t ui32Token) {
  return (ui32Token & 0x000000f0) >> 4;
}

inline static uint32_t DecodeProgramMinorVersion(uint32_t ui32Token) {
  return (ui32Token & 0x0000000f);
}

inline static uint32_t DecodeInstructionLength(uint32_t ui32Token) {
  return (ui32Token & 0x7f000000) >> 24;
}

inline static uint32_t DecodeIsOpcodeExtended(uint32_t ui32Token) {
...
static RESOURCE_RETURN_TYPE DecodeResourceReturnType(uint32_t ui32Coord,
                                                     uint32_t ui32Token) {
  return (RESOURCE_RETURN_TYPE)((ui32Token >> (ui32Coord * 4)) & 0xF);
}

static RESOURCE_RETURN_TYPE
DecodeExtendedResourceReturnType(uint32_t ui32Coord, uint32_t ui32Token) {
...
typedef enum OPERAND_TYPE {
  ...
  OPERAND_TYPE_RASTERIZER = 14, // DX10.1 Rasterizer register, used to denote
                                // the depth/stencil and render target resources
...
DecodeOperandIndexRepresentation(uint32_t ui32Dimension, uint32_t ui32Token) {
  return (
      OPERAND_INDEX_REPRESENTATION)((ui32Token &
                                     (0x7 << (22 + 3 * ((ui32Dimension)&3)))) >>
...
static OPERAND_4_COMPONENT_SELECTION_MODE
DecodeOperand4CompSelMode(uint32_t ui32Token) {
...
static const uint32_t NO_SWIZZLE =
    (((OPERAND_4_COMPONENT_X) | (OPERAND_4_COMPONENT_Y << 2) |
      (OPERAND_4_COMPONENT_Z << 4) | (OPERAND_4_COMPONENT_W << 6)) /*<<4*/);

static const uint32_t XXXX_SWIZZLE =
    (((OPERAND_4_COMPONENT_X) | (OPERAND_4_COMPONENT_X << 2) |
      (OPERAND_4_COMPONENT_X << 4) | (OPERAND_4_COMPONENT_X << 6)));
static const uint32_t YYYY_SWIZZLE =
    (((OPERAND_4_COMPONENT_Y) | (OPERAND_4_COMPONENT_Y << 2) |
      (OPERAND_4_COMPONENT_Y << 4) | (OPERAND_4_COMPONENT_Y << 6)));
static const uint32_t ZZZZ_SWIZZLE =
    (((OPERAND_4_COMPONENT_Z) | (OPERAND_4_COMPONENT_Z << 2) |
      (OPERAND_4_COMPONENT_Z << 4) | (OPERAND_4_COMPONENT_Z << 6)));
static const uint32_t WWWW_SWIZZLE =
    (((OPERAND_4_COMPONENT_W) | (OPERAND_4_COMPONENT_W << 2) |
      (OPERAND_4_COMPONENT_W << 4) | (OPERAND_4_COMPONENT_W << 6)));

static uint32_t DecodeOperand4CompSwizzleSource(uint32_t ui32Token,
                                                uint32_t comp) {
...
static uint32_t
DecodeInstructionPreciseMask(uint32_t ui32Token) // "precise" keyword
{
...
#define IMMEDIATE_ADDRESS_OFFSET_MASK(Coord)                                   \
  (0x0000000f << IMMEDIATE_ADDRESS_OFFSET_SHIFT(Coord))

static uint32_t
DecodeImmediateAddressOffset(IMMEDIATE_ADDRESS_OFFSET_COORD eCoord,
                             uint32_t ui32Token) {
  return ((((ui32Token)&IMMEDIATE_ADDRESS_OFFSET_MASK(eCoord)) >>
           (IMMEDIATE_ADDRESS_OFFSET_SHIFT(eCoord))));
}
