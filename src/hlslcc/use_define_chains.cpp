...
#if DEBUG_UDCHAINS
// Debug mode
static void UDCheckConsistencyDUChain(uint32_t idx, DefineUseChains &psDUChains,
                                      UseDefineChains &psUDChains,
  ...
  while (du != psDUChains[idx].end()) {
      ...
      for (i = 0; i < du->psInst->ui32FirstSrc; i++) {
        if (du->psInst->asOperands[i].eType == OPERAND_TYPE_TEMP) {
          if (du->psInst->asOperands[i].ui32RegisterNumber == tempReg) {
            uint32_t writeMask =
                GetOperandWriteMask(&du->psInst->asOperands[i]);
    ...
    // Check that each usage of each definition also is found in the use-define
    // chain
    ...
    while (ul != du->usages.end()) {
      ...
      ASSERT(std::find(use->defines.begin(), use->defines.end(), &*du) !=
             use->defines.end());
...
static void UDCheckConsistencyUDChain(uint32_t idx, DefineUseChains &psDUChains,
                                      UseDefineChains &psUDChains,
                                      ActiveDefinitions &activeDefinitions) {
  DefineUseChain::iterator du = psDUChains[idx].begin();
  UseDefineChain::iterator ud = psUDChains[idx].begin();
  while (ud != psUDChains[idx].end()) {
    // Check that each definition of each usage also is found in the define-use
    // chain
    ...
    while (dl != ud->defines.end()) {
      ...
      // Check that the mapping back is also found
      ASSERT(std::find(def->usages.begin(), def->usages.end(), &*ud) !=
             def->usages.end());
...
static void UDCheckConsistency(uint32_t tempRegs, DefineUseChains &psDUChains,
                               UseDefineChains &psUDChains,
                               ActiveDefinitions &activeDefinitions) {
  uint32_t i;
...
static DefineUseChainEntry *
GetOrCreateDefinition(const BasicBlock::Definition &def,
                      DefineUseChain &psDUChain, uint32_t index) {
  // Try to find an existing entry
  auto itr = std::find_if(
      psDUChain.begin(), psDUChain.end(), [&](const DefineUseChainEntry &de) {
...
// Do flow control analysis on the instructions and build the define-use and
// use-define chains
void BuildUseDefineChains(std::vector<Instruction> &instructions,
                          uint32_t ui32NumTemps, DefineUseChains &psDUChain,
                          UseDefineChains &psUDChain,
                          dxmt::ControlFlow::ControlFlowGraph &cfg) {
  ActiveDefinitions lastSeenDefinitions(
      ui32NumTemps * 4, NULL); // Array of pointers to the currently active
                               // definition for each temp
  ...
  // Loop through each block, first calculate the union of all the reachables of
  // all preceding blocks and then build on that as we go along the basic block
  // instructions
  for_each(
      blocks.begin(), blocks.end(),
      [&](const std::shared_ptr<BasicBlock> &bptr) {
        const BasicBlock &b = *bptr.get();
        BasicBlock::ReachableVariables rvars;
        for_each(b.Preceding().begin(), b.Preceding().end(),
                 [&](const Instruction *precBlock) {
                   const BasicBlock &b =
                       *cfg.GetBasicBlockForInstruction(precBlock);
                   BasicBlock::RVarUnion(rvars, b.Reachable());
                 });

        // Now we have a Reachable set for the beginning of this block in rvars.
        // Loop through all instructions and their operands and pick up uses and
        // definitions
        for (const Instruction *inst = b.First(); inst <= b.Last(); inst++) {
          // Process sources first
          ForEachOperand(
              inst, inst + 1, FEO_FLAG_SRC_OPERAND | FEO_FLAG_SUBOPERAND,
              [&](const Instruction *psInst, const Operand *psOperand,
                ...
                for (int k = 0; k < 4; k++) {
                  ...
                  BasicBlock::ReachableDefinitionsPerVariable &rpv =
                      rvars[regIdx];
                  for_each(rpv.begin(), rpv.end(),
                           [&](const BasicBlock::Definition &def) {
                             DefineUseChainEntry *duentry =
                                 GetOrCreateDefinition(def, psDUChain[regIdx],
                                                       k);
          ...
          ForEachOperand(inst, inst + 1, FEO_FLAG_DEST_OPERAND,
                         [&](const Instruction *psInst,
                           ...
                           for (int k = 0; k < 4; k++) {
                             ...
                             // Overwrite whatever's in rvars; they are killed
                             // by this
                             rvars[regIdx].clear();
                             rvars[regIdx].insert(
                                 BasicBlock::Definition(psInst, psOperand));

                             // Make sure the definition gets created even
                             // though it doesn't have any uses at all (happens
                             // when sampling a texture but not all channels are
                             // used etc).
  ...
  for_each(psUDChain.begin(), psUDChain.end(),
           [&](std::pair<const uint32_t, UseDefineChain> &udpair) {
             ...
             for_each(ud.begin(), ud.end(), [&](UseDefineChainEntry &ue) {
               ...
               // Go through each component
               for (int k = 0; k < 4; k++) {
                 // Skip components that we don't access, or the one that's our
                 // own
                 if (!(ue.accessMask & (1 << k)) || ue.index == k)
                   continue;

                 // Find the corresponding sibling. We can uniquely identify it
                 // by the operand pointer alone.
                 UseDefineChain::iterator siblItr = std::find_if(
                     psUDChain[baseReg + k].begin(),
                     psUDChain[baseReg + k].end(),
                     [&](const UseDefineChainEntry &_sibl) -> bool {
                       return _sibl.psOp == ue.psOp;
                     });
  ...
  for_each(psDUChain.begin(), psDUChain.end(),
           [&](std::pair<const uint32_t, DefineUseChain> &dupair) {
             ...
             for_each(du.begin(), du.end(), [&](DefineUseChainEntry &de) {
               ...
               for (int k = 0; k < 4; k++) {
                 ...
                 // Find the corresponding sibling. We can uniquely identify it
                 // by the operand pointer alone.
                 DefineUseChain::iterator siblItr = std::find_if(
                     psDUChain[baseReg + k].begin(),
                     psDUChain[baseReg + k].end(),
                     [&](const DefineUseChainEntry &_sibl) -> bool {
                       return _sibl.psOp == de.psOp;
                     });
...
static void UDDoSplit(SplitDefinitions &defs, uint32_t *psNumTemps,
                      DefineUseChains &psDUChains, UseDefineChains &psUDChains,
                      std::vector<uint32_t> &pui32SplitTable) {
...
#if DEBUG_UDCHAINS
  UDCheckConsistency((*psNumTemps) - 1, psDUChains, psUDChains,
                     ActiveDefinitions());
#endif
  ...
  for (u32def = 0; u32def < defs.size(); u32def++) {
...
#if DEBUG_UDCHAINS
    printf("Split def at instruction %d (reg %d -> %d, access %X, rebase %d, "
           "count: %d)\n",
           (int)defineToSplit->psInst->id, oldReg, newReg, accessMask, rebase,
           count);
#endif

    // We may have moved the opcodes already because of multiple defines
    // pointing to the same op
    if (defineToSplit->psOp->ui32RegisterNumber != newReg) {
      ...
      defineToSplit->psInst->ChangeOperandTempRegister(
          defineToSplit->psOp, oldReg, newReg, accessMask,
          UD_CHANGE_MAIN_OPERAND, rebase);
    }
    ...
    while (ul != defineToSplit->usages.end()) {
      ...
      if ((*ul)->psOp->ui32RegisterNumber != newReg) {
        ...
        (*ul)->psInst->ChangeOperandTempRegister(
            (*ul)->psOp, oldReg, newReg, accessMask, UD_CHANGE_MAIN_OPERAND,
            rebase);
      }

      // Update the UD chain
      {
        UseDefineChain::iterator udLoc =
            psUDChains[oldReg * 4 + oldIdx].begin();
        while (udLoc != psUDChains[oldReg * 4 + oldIdx].end()) {
          if (&*udLoc == *ul) {
            // Move to new list
            psUDChains[newReg * 4 + oldIdx - rebase].splice(
                psUDChains[newReg * 4 + oldIdx - rebase].begin(),
                psUDChains[oldReg * 4 + oldIdx], udLoc);

            if (rebase > 0) {
              (*ul)->accessMask >>= rebase;
              (*ul)->index -= rebase;
              memmove((*ul)->psSiblings, (*ul)->psSiblings + rebase,
                      (4 - rebase) * sizeof(UseDefineChain *));
            }
            break;
          }
          udLoc++;
    ...
    // Move the define out of the old chain (if its still there)
    {
      ...
      while (duLoc != psDUChains[oldReg * 4 + oldIdx].end() &&
             ((&*duLoc) != defineToSplit)) {
        duLoc++;
      }
      ASSERT(duLoc != psDUChains[oldReg * 4 + oldIdx].end());
      {
        ...
        psDUChains[newReg * 4 + oldIdx - rebase].splice(
            psDUChains[newReg * 4 + oldIdx - rebase].begin(),
            psDUChains[oldReg * 4 + oldIdx], duLoc);
        if (rebase != 0) {
          memmove(defineToSplit->psSiblings, defineToSplit->psSiblings + rebase,
                  (4 - rebase) * sizeof(DefineUseChain *));
        }
      }
    }
  }
...
static void AddDefineToList(SplitDefinitions &defs,
                            DefineUseChainEntry *newDef) {
  uint32_t k;
  for (k = 0; k < 4; k++) {
...
// Check if a set of definitions can be split and does the split. Returns
// nonzero if a split took place
static int AttemptSplitDefinitions(SplitDefinitions &defs, uint32_t *psNumTemps,
                                   DefineUseChains &psDUChains,
                                   UseDefineChains &psUDChains,
                                   std::vector<uint32_t> &pui32SplitTable) {
  uint32_t reg;
  ...
  // Have at least one definition in any of the 4 register slots that isn't
  // included
  if (defs.empty())
    return 0;
  ...
  // We'd be splitting the entire register and all its definitions, no point in
  // that.
  if (hasLeftoverDefinitions == 0)
    return 0;

  // Check all the definitions. Any of them must not have any usages that see
  // any definitions not in our defs array.
  for (u32def = 0; u32def < defs.size(); u32def++) {
    ...
    while (ul != def->usages.end()) {
      ...
      if (((*ul)->accessMask & (~combinedMask)) != 0) {
        ...
        return AttemptSplitDefinitions(defs, psNumTemps, psDUChains, psUDChains,
                                       pui32SplitTable);
      }
      ...
      for (j = 0; j < 4; j++) {
        ...
        // Check that all definitions for this usage are found from the
        // definitions table
        ...
        while (dl != (*ul)->psSiblings[j]->defines.end()) {
          ...
          if (defineFound == 0) {
            ...
            return AttemptSplitDefinitions(defs, psNumTemps, psDUChains,
                                           psUDChains, pui32SplitTable);
          }
...
void UDSplitTemps(uint32_t *psNumTemps, DefineUseChains &psDUChains,
                  UseDefineChains &psUDChains,
                  std::vector<uint32_t> &pui32SplitTable) {
  // Algorithm overview:
  // Take each definition and look at all its usages. If all usages only see
  // this definition (and this is not the only definition for this variable),
  // split it out.
  uint32_t i;
  uint32_t tempsAtStart =
      *psNumTemps; // We don't need to try to analyze the newly created ones,
                   // they're unsplittable by definition
  for (i = 0; i < tempsAtStart * 4; i++) {
    ...
    while (du != psDUChains[i].end()) {
      ...
      du++;
      // If we split, we'll have to start from the beginning of this chain
      // because du might no longer be in this chain
      if (AttemptSplitDefinitions(sd, psNumTemps, psDUChains, psUDChains,
                                  pui32SplitTable)) {
...
// Returns true if all the usages of this definitions are instructions that deal
...
void CalculateStandaloneDefinitions(DefineUseChains &psDUChains,
    ...
    while (du != psDUChains[i].end()) {
      ...
      for (sibl = 0; sibl < 4; sibl++) {
        ...
        while (ul != du->psSiblings[sibl]->usages.end()) {
          ...
          // Need to check that all the siblings of this usage only see this
          // definition's corresponding sibling
          for (k = 0; k < 4; k++) {
            if (!(*ul)->psSiblings[k])
              continue;
...
void WriteBackUsesAndDefines(DefineUseChains &psDUChains) {
  // Loop through the whole data structure, and write usages and defines to
  // Instructions and Operands as we see them
  for_each(psDUChains.begin(), psDUChains.end(),
           [](const DefineUseChains::value_type &itr) {
             const DefineUseChain &duChain = itr.second;
             for_each(duChain.begin(), duChain.end(),
                      [](const DefineUseChain::value_type &du) {
                        for_each(du.usages.begin(), du.usages.end(),
                                 [&du](const UseDefineChainEntry *usage) {
                                   // Update instruction use list
                                   du.psInst->m_Uses.push_back(Instruction::Use(
                                       usage->psInst, usage->psOp));
                                   // And the usage's definition
                                   usage->psOp->m_Defines.push_back(
                                       Operand::Define(du.psInst, du.psOp));
                                 });
                      });
           });
}
