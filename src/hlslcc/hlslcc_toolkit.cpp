...
const char *GetConstructorForTypeMetal(const SHADER_VARIABLE_TYPE eType,
  ...
  static const char *const uintTypes[] = {" ", "uint", "uint2", "uint3",
                                          "uint4"};
  static const char *const ushortTypes[] = {" ", "ushort", "ushort2", "ushort3",
                                            "ushort4"};
  static const char *const intTypes[] = {" ", "int", "int2", "int3", "int4"};
  static const char *const shortTypes[] = {" ", "short", "short2", "short3",
                                           "short4"};
  static const char *const floatTypes[] = {" ", "float", "float2", "float3",
                                           "float4"};
  static const char *const halfTypes[] = {" ", "half", "half2", "half3",
                                          "half4"};
  static const char *const boolTypes[] = {" ", "bool", "bool2", "bool3",
                                          "bool4"};
...
const char *GetConstructorForType(const HLSLCrossCompilerContext *psContext,
                                  const SHADER_VARIABLE_TYPE eType,
                                  const int components) {

  return GetConstructorForTypeMetal(eType, components);
}

std::string GetMatrixTypeName(const HLSLCrossCompilerContext *psContext,
                              const SHADER_VARIABLE_TYPE eBaseType,
...
// Returns true if we can do direct assignment between types (mostly for
// mediump<->highp floats etc)
bool DoAssignmentDataTypesMatch(SHADER_VARIABLE_TYPE dest,
                                SHADER_VARIABLE_TYPE src) {
  if (src == dest)
    return true;
...
ResourceReturnTypeToSVTType(const RESOURCE_RETURN_TYPE eType) {
  if (eType == RETURN_TYPE_SINT) {
...
// Returns true if operands are identical, only cares about temp registers
// currently.
bool AreTempOperandsIdentical(const Operand *psA, const Operand *psB) {
  ...
  if (psA->eSelMode == OPERAND_4_COMPONENT_MASK_MODE &&
      psA->ui32CompMask != psB->ui32CompMask)
    return 0;

  if (psA->eSelMode == OPERAND_4_COMPONENT_SELECT_1_MODE &&
      psA->aui32Swizzle[0] != psB->aui32Swizzle[0])
    return 0;

  if (psA->eSelMode == OPERAND_4_COMPONENT_SWIZZLE_MODE &&
      std::equal(&psA->aui32Swizzle[0], &psA->aui32Swizzle[4],
                 &psB->aui32Swizzle[0]))
    return 0;
...
bool IsAddOneInstruction(const Instruction *psInst) {
  if (psInst->eOpcode != OPCODE_IADD)
    return false;
  if (psInst->operands[0].eType != OPERAND_TYPE_TEMP)
    return false;

  if (psInst->operands[1].eType == OPERAND_TYPE_TEMP) {
    if (psInst->operands[1].ui32RegisterNumber !=
        psInst->operands[0].ui32RegisterNumber)
      return false;
    if (psInst->operands[2].eType != OPERAND_TYPE_IMMEDIATE32)
      return false;

    if (*(int *)&psInst->operands[2].afImmediates[0] != 1)
      return false;
  } else {
    if (psInst->operands[1].eType != OPERAND_TYPE_IMMEDIATE32)
      return false;
    if (psInst->operands[2].eType != OPERAND_TYPE_TEMP)
      return false;

    if (psInst->operands[2].ui32RegisterNumber !=
        psInst->operands[0].ui32RegisterNumber)
      return false;

    if (*(int *)&psInst->operands[1].afImmediates[0] != 1)
      return false;
  }
  return true;
}
...
SHADER_VARIABLE_TYPE SelectHigherType(SHADER_VARIABLE_TYPE a,
                                      SHADER_VARIABLE_TYPE b) {
...
bool CanDoDirectCast(const HLSLCrossCompilerContext *context,
                     SHADER_VARIABLE_TYPE src, SHADER_VARIABLE_TYPE dest) {
  // uint<->int<->bool conversions possible
  if ((src == SVT_INT || src == SVT_UINT || src == SVT_BOOL ||
       src == SVT_INT12 || src == SVT_INT16 || src == SVT_UINT16) &&
      (dest == SVT_INT || dest == SVT_UINT || dest == SVT_BOOL ||
       dest == SVT_INT12 || dest == SVT_INT16 || dest == SVT_UINT16))
    return true;

  // float<->double possible
  if ((src == SVT_FLOAT || src == SVT_DOUBLE || src == SVT_FLOAT16 ||
       src == SVT_FLOAT10) &&
      (dest == SVT_FLOAT || dest == SVT_DOUBLE || dest == SVT_FLOAT16 ||
       dest == SVT_FLOAT10))
    return true;

  {
    // avoid compiler error: cannot use as_type to cast from 'half' to 'unsigned
    // int' or 'int', types of different size
    if ((src == SVT_FLOAT16 || src == SVT_FLOAT10) &&
        (dest == SVT_UINT || dest == SVT_INT))
      return true;
  }
...
bstring GetEarlyMain(HLSLCrossCompilerContext *psContext) {
  bstring *oldString = psContext->currentShaderString;
  bstring *str =
  ...
  psContext->currentShaderString = str;
...
bstring GetPostShaderCode(HLSLCrossCompilerContext *psContext) {
  ...
  psContext->psShader->phases[psContext->currentPhase].hasPostShaderCode = 1;
