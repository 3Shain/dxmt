...
Converter::LoadOperand(const SrcOperandAttribute &SrcOp, mask_t Mask) {
  ...
  case InputAttribute::ThreadId: {
    Ret = ctx.resource.thread_id_arg;
    break;
  }
  case InputAttribute::ThreadIdInGroup: {
    Ret = ctx.resource.thread_id_in_group_arg;
    break;
  }
  case InputAttribute::ThreadGroupId: {
    Ret = ctx.resource.thread_group_id_arg;
    break;
  }
  case InputAttribute::ThreadIdInGroupFlatten: {
    Ret = ctx.resource.thread_id_in_group_flat_arg;
    break;
  }
  ...
  case InputAttribute::CoverageMask: {
    Ret = ctx.pso_sample_mask != 0xffffffff ? ir.CreateAnd(ctx.resource.coverage_mask_arg, ctx.pso_sample_mask)
                                            : ctx.resource.coverage_mask_arg;
    break;
  }
  case InputAttribute::Domain: {
    Ret = ctx.resource.domain;
    break;
  }
