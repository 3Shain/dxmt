...
TextureOperationInfo TextureInfo[] = {
  ...
  // texture1d_array,
  {"texture_1d_array", true, 1, false, false, false,
   true /* but only 0 is valid */},
  // texture2d,
...
std::string AIRBuilder::getTypeOverloadSuffix(Type *Ty, Signedness Sign) {
  std::string Ret = ".";
...
Value *AIRBuilder::CreateAtomicRMW(
  const Texture &Texture, Value *Handle, AtomicRMWInst::BinOp Op, Value *Pos,
  Value *ValVec4, Value *ArrayIndex
) {
  ...
  if (Texture.sample_type != Texture::sample_int &&
      Texture.sample_type != Texture::sample_uint) {
  ...
  if (Op == llvm::AtomicRMWInst::Xchg) {
    ...
    FnName +=
      getTypeOverloadSuffix(getTexelType(Texture), Signedness::Unsigned);
  } else {
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getTexelType(Texture), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

Value *AIRBuilder::CreateAtomicCmpXchg(
  const Texture &Texture, Value *Handle, Value *Pos, Value *CmpVec4,
  Value *NewVec4, Value *ArrayIndex
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
  ...
  FnName += TexInfo.air_symbol_suffix;
  FnName += getTypeOverloadSuffix(getBoolTy(), Signedness::Unsigned);

  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getTexelType(Texture), Tys, false), Attrs
  );

  // comparison result ignored
  builder.CreateCall(Fn, Ops);

  return builder.CreateLoad(TyTexel, Ptr);
}

Type *AIRBuilder::getTextureHandleType(const Texture &Texture) {
  assert(Texture.kind < Texture::num_resource_kind);
  return getOrCreateStructType(std::format(
                                 "struct._{}_t",
                                 TextureInfo[Texture.kind].air_symbol_suffix
                               ))
    ->getPointerTo(1);
};

Type *AIRBuilder::getTexelType(const Texture &Texture) {
  assert(Texture.kind < Texture::num_resource_kind);
...
Type *AIRBuilder::getTexelGatherType(const Texture &Texture) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &info = TextureInfo[Texture.kind];
  if (info.is_depth)
...
Type *AIRBuilder::getTextureSampleCoordType(const Texture &Texture) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &info = TextureInfo[Texture.kind];
  return getFloatTy(info.coord_dimension);
}

Type *AIRBuilder::getTextureRWPositionType(const Texture &Texture) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &info = TextureInfo[Texture.kind];
  if (info.is_cube) {
    return getIntTy(2);
...
std::pair<Value *, Value *> AIRBuilder::CreateSampleCommon(
  const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
  Value *ArrayIndex, const std::array<int32_t, 3> &Offset, bool ArgsControlBit,
  Value *Args1, Value *Args2
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getTextureSampleResultType(Texture), Tys, false),
    Attrs
  );
  ...
  return {
    ...
    builder.CreateExtractValue(Result, {1u})
  };
};

std::pair<Value *, Value *> AIRBuilder::CreateSampleCmpCommon(
  const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
  Value *ArrayIndex, Value *Reference, const std::array<int32_t, 3> &Offset,
...
std::pair<Value *, Value *> AIRBuilder::CreateSampleGrad(
  const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
  Value *ArrayIndex, Value *DerivX, Value *DerivY, Value *MinLOD,
...
std::pair<Value *, Value *> AIRBuilder::CreateRead(
  const Texture &Texture, Value *Handle, Value *Pos, Value *ArrayIndex,
  Value *SampleIndexOrCubeFace, Value *Level
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
...
CallInst *AIRBuilder::CreateWrite(
  const Texture &Texture, Value *Handle, Value *Pos, Value *ArrayIndex,
  Value *CubeFace, Value *Level, Value *ValVec4
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getVoidTy(), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

std::pair<Value *, Value *> AIRBuilder::CreateGather(
  const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
  Value *ArrayIndex, const std::array<int32_t, 3> &Offset, Value *Component
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
  ...
  FnName += TexInfo.air_symbol_suffix;
  FnName +=
    getTypeOverloadSuffix(getTexelGatherType(Texture), getTexelSign(Texture));

  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getTextureGatherResultType(Texture), Tys, false),
    Attrs
  );
...
std::pair<Value *, Value *> AIRBuilder::CreateGatherCompare(
  const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
  Value *ArrayIndex, Value *Reference, const std::array<int32_t, 3> &Offset
) {
  assert(Texture.kind < Texture::num_resource_kind);
...
Value *AIRBuilder::CreateTextureQuery(
  const Texture &Texture, Value *Handle, Texture::Query Query, Value *Level
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
  ...
  static const char *QUERYS[] = {
    ...
    "get_array_size", "get_num_mip_levels", "get_num_samples",
  };
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getIntTy(), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

std::pair<Value *, Value *> AIRBuilder::CreateCalculateLOD(
  const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord
) {
  assert(Texture.kind < Texture::num_resource_kind);
  auto &TexInfo = TextureInfo[Texture.kind];
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(getFloatTy(), Tys, false), Attrs
  );

  std::string FnUnclampedName = "air.calculate_unclamped_lod_";
  FnUnclampedName += TexInfo.air_symbol_suffix;

  auto FnUnclamped = getModule()->getOrInsertFunction(
    FnUnclampedName, FunctionType::get(getFloatTy(), Tys, false), Attrs
  );

  return {builder.CreateCall(Fn, Ops), builder.CreateCall(FnUnclamped, Ops)};
}

Value *AIRBuilder::CreateGetNumSamples() {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context,
    {
  ...
  auto Fn = getModule()->getOrInsertFunction(
    "air.get_num_samples.i32", FunctionType::get(getIntTy(), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

Value *AIRBuilder::CreateDerivative(Value *Val, bool YAxis) {
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(Val->getType(), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

CallInst *AIRBuilder::CreateDiscard() {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::WillReturn)}}
  ...
  auto Fn = getModule()->getOrInsertFunction(
    "air.discard_fragment", FunctionType::get(getVoidTy(), {}, false), Attrs
  );

  return builder.CreateCall(Fn, {});
}

CallInst *AIRBuilder::CreateBarrier(MemFlags Flags) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
  ...
  auto Fn = getModule()->getOrInsertFunction(
    "air.wg.barrier", FunctionType::get(getVoidTy(), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

Value *AIRBuilder::CreateFMA(Value *X, Value *Y, Value *Z) {
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(X->getType(), Tys, false), Attrs
  );

  return builder.CreateCall(Fn, Ops);
}

Value *AIRBuilder::CreateDotProduct(Value *LHS, Value *RHS) {
...
Value *AIRBuilder::CreateCountZero(Value *Val, bool TrailingZero) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{~0U, Attribute::get(Context, Attribute::AttrKind::ReadNone)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
...
Value *AIRBuilder::CreateConvertToFloat(Value *Val, Signedness SrcSign) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{~0U, Attribute::get(Context, Attribute::AttrKind::ReadNone)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  Type *TyDst = isa<FixedVectorType>(TySrc)
                  ? getFloatTy(cast<FixedVectorType>(TySrc)->getNumElements())
                  : getFloatTy();
  ...
  auto Fn = getModule()->getOrInsertFunction(
    FnName, FunctionType::get(TyDst, {TySrc}, false), Attrs
  );

  return builder.CreateCall(Fn, {Val});
}

Value *AIRBuilder::CreateConvertToHalf(Value *Val, Signedness SrcSign) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{~0U, Attribute::get(Context, Attribute::AttrKind::ReadNone)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  Type *TySrc = Val->getType();
  Type *TyDst = isa<FixedVectorType>(TySrc)
                  ? getHalfTy(cast<FixedVectorType>(TySrc)->getNumElements())
                  : getHalfTy();
...
Value *AIRBuilder::CreateConvertToSigned(Value *Val) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{~0U, Attribute::get(Context, Attribute::AttrKind::ReadNone)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  Type *TySrc = Val->getType();
  Type *TyDst = isa<FixedVectorType>(TySrc)
                  ? getIntTy(cast<FixedVectorType>(TySrc)->getNumElements())
                  : getIntTy();
...
Value *AIRBuilder::CreateConvertToUnsigned(Value *Val) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{~0U, Attribute::get(Context, Attribute::AttrKind::ReadNone)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
...
Value *AIRBuilder::CreateInterpolateAtSample(
  Value *Interpoant, Value *SampleIndex, bool Perspective
) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getFloatTy(4), {Interpoant->getType(), getIntTy()}, false
    ),
    Attrs
  );
...
Value *AIRBuilder::CreateInterpolateAtOffset(
  Value *Interpoant, Value *Offset, bool Perspective
) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getFloatTy(4), {Interpoant->getType(), getIntTy(2)}, false
    ),
    Attrs
  );
...
CallInst *AIRBuilder::CreateSetMeshProperties(Value *GridSizeVec3) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getVoidTy(), {getMeshGridPropsType(), getIntTy(3)}, false
    ),
    Attrs
  );
...
CallInst *AIRBuilder::CreateSetMeshRenderTargetArrayIndex(
  Value *Vertex, Value *ArrayIndex
) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getVoidTy(), {getMeshHandleType(), getIntTy(), getIntTy()}, false
    ),
    Attrs
  );
...
CallInst *AIRBuilder::CreateSetMeshPosition(Value *Vertex, Value *Position) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getVoidTy(), {getMeshHandleType(), getIntTy(), getFloatTy(4)}, false
    ),
    Attrs
  );
...
CallInst *AIRBuilder::CreateSetMeshClipDistance(
  Value *Vertex, Value *Index, Value *Value
) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getVoidTy(), {getMeshHandleType(), getIntTy(), getIntTy(), getFloatTy()},
      false
    ),
    Attrs
  );
...
CallInst *AIRBuilder::CreateSetMeshVertexData(
  Value *Vertex, Value *DataIndex, Value *DataValue
) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(
      getVoidTy(),
      {getMeshHandleType(), getIntTy(), getIntTy(), DataValue->getType()}, false
    ),
    Attrs
  );

  return builder.CreateCall(
    Fn, {getMeshHandle(), DataIndex, Vertex, DataValue}
  );
}

CallInst *AIRBuilder::CreateSetMeshPrimitiveData(
  ...
  return builder.CreateCall(
    Fn, {getMeshHandle(), DataIndex, Primitive, DataValue}
  );
}

CallInst *AIRBuilder::CreateSetMeshIndex(Value *Index, Value *Vertex) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    "air.set_index_mesh",
    FunctionType::get(
      getVoidTy(), {getMeshHandleType(), getIntTy(), getByteTy()}, false
    ),
    Attrs
  );

  return builder.CreateCall(
    Fn, {getMeshHandle(), Index, builder.CreateZExtOrTrunc(Vertex, getByteTy())}
  );
}

CallInst *AIRBuilder::CreateSetMeshPrimitiveCount(Value *Count) {
  auto &Context = getContext();
  auto Attrs = AttributeList::get(
    Context, {{1U, Attribute::get(Context, Attribute::AttrKind::NoCapture)},
              {~0U, Attribute::get(Context, Attribute::AttrKind::NoUnwind)},
  ...
  auto Fn = getModule()->getOrInsertFunction(
    ...
    FunctionType::get(getVoidTy(), {getMeshHandleType(), getIntTy()}, false),
    Attrs
  );
