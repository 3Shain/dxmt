...
class AIRBuilder {
public:
  AIRBuilder(IRBuilderBase &ir_builder, raw_ostream &debug)
      : builder(ir_builder), debug(debug) {}

  /* Texture Operations */

  std::pair<Value *, Value *> CreateSample(
    const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
    Value *ArrayIndex, const std::array<int32_t, 3> &Offset
  ) {
    return CreateSampleCommon(
      Texture, Handle, Sampler, Coord, ArrayIndex, Offset, false, getFloat(0),
      getFloat(0)
    );
  }
  ...
  std::pair<Value *, Value *> CreateSample(
    const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
    Value *ArrayIndex, const std::array<int32_t, 3> &Offset, sample_level Level
  ) {
    return CreateSampleCommon(
      Texture, Handle, Sampler, Coord, ArrayIndex, Offset, true, Level.lod,
      getFloat(0)
    );
  }
  ...
  std::pair<Value *, Value *> CreateSample(
    const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
    Value *ArrayIndex, const std::array<int32_t, 3> &Offset, sample_bias Bias
  ) {
    return CreateSampleCommon(
      Texture, Handle, Sampler, Coord, ArrayIndex, Offset, false, Bias.bias,
      getFloat(0)
    );
  }
  ...
  std::pair<Value *, Value *> CreateSampleCmp(
    const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
    ...
    return CreateSampleCmpCommon(
      Texture, Handle, Sampler, Coord, ArrayIndex, Reference, Offset, false,
      Bias.bias, MinLOD.lod
    );
  }
  ...
  std::pair<Value *, Value *> CreateSampleCmp(
    ...
    return CreateSampleCmpCommon(
      Texture, Handle, Sampler, Coord, ArrayIndex, Reference, Offset, true,
      Level.lod, getFloat(0)
    );
  }

  std::pair<Value *, Value *> CreateSampleGrad(
    const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord,
    Value *ArrayIndex, Value *DerivX, Value *DerivY,
    const std::array<int32_t, 3> &Offset
  ) {
    return CreateSampleGrad(
      Texture, Handle, Sampler, Coord, ArrayIndex, DerivX, DerivY, getFloat(0),
      Offset
    );
  }
  ...
  Value *CreateAtomicRMW(
    ...
    Value *ValVec4, Value *ArrayIndex = nullptr
  );
  ...
  Value *CreateAtomicCmpXchg(
    const Texture &Texture, Value *Handle, Value *Pos, Value *CmpVec4,
    Value *NewVec4, Value *ArrayIndex = nullptr
  );
  ...
  */
  std::pair<Value *, Value *> CreateCalculateLOD(
    const Texture &Texture, Value *Handle, Value *Sampler, Value *Coord
  );
  ...
  Value *CreateInterpolateAtSample(
    Value *Interpoant, Value *SampleIndex, bool Perspective
  );

  Value *
  CreateInterpolateAtOffset(Value *Interpoant, Value *Offset, bool Perspective);
  ...
  CallInst *
  CreateSetMeshRenderTargetArrayIndex(Value *Vertex, Value *ArrayIndex);
  ...
  CallInst *
  CreateSetMeshClipDistance(Value *Vertex, Value *Index, Value *Value);
  ...
  ConstantInt *getInt(uint32_t Value) { return builder.getInt32(Value); }
  ConstantVector *getInt2(uint32_t Value1, uint32_t Value2) {
    return cast<ConstantVector>(
      ConstantVector::get({builder.getInt32(Value1), builder.getInt32(Value2)})
    );
  }
  ConstantVector *getInt3(uint32_t Value1, uint32_t Value2, uint32_t Value3) {
    return cast<ConstantVector>(ConstantVector::get(
      {builder.getInt32(Value1), builder.getInt32(Value2),
       builder.getInt32(Value3)}
    ));
  }
  ConstantVector *
  getInt4(uint32_t Value1, uint32_t Value2, uint32_t Value3, uint32_t Value4) {
    return cast<ConstantVector>(ConstantVector::get(
      {builder.getInt32(Value1), builder.getInt32(Value2),
       builder.getInt32(Value3), builder.getInt32(Value4)}
    ));
  }
  ...
  Type *getHalfTy() { return Type::getHalfTy(getContext()); };
  Type *getIntTy(uint32_t dim) {
    assert(dim > 0 && dim <= 4);
    if (dim == 1)
      return getIntTy();
    return FixedVectorType::get(getIntTy(), dim);
  };
  Type *getFloatTy(uint32_t dim) {
    assert(dim > 0 && dim <= 4);
    if (dim == 1)
      return getFloatTy();
    return FixedVectorType::get(getFloatTy(), dim);
  };
  Type *getHalfTy(uint32_t dim) {
    assert(dim > 0 && dim <= 4);
    if (dim == 1)
      return getHalfTy();
    return FixedVectorType::get(getHalfTy(), dim);
  };
  Type *getBoolTy() { return Type::getInt1Ty(getContext()); };
  Type *getVoidTy() { return Type::getVoidTy(getContext()); };
  Type *getByteTy() { return Type::getInt8Ty(getContext()); };
  ...
  Type *getTextureSampleResultType(const Texture &Texture) {
    return StructType::get(
      getContext(), {getTexelType(Texture), Type::getInt8Ty(getContext())}
    );
  }

  Type *getTextureGatherResultType(const Texture &Texture) {
    return StructType::get(
      getContext(), {getTexelGatherType(Texture), Type::getInt8Ty(getContext())}
    );
  }

  Type *getSamplerHandleType() {
    return getOrCreateStructType("struct._sampler_t")->getPointerTo(2);
  }
  Type *getMeshHandleType() {
    return getOrCreateStructType("struct._mesh_t")->getPointerTo(7);
  }
  Type *getMeshGridPropsType() {
    return getOrCreateStructType("struct._mesh_grid_properties_t")
      ->getPointerTo(3);
  }
...
protected:
  ...
  std::string
  getTypeOverloadSuffix(Type *Ty, Signedness Sign = Signedness::DontCare);

  Signedness getTexelSign(const Texture &Texture) {
