...
#else
#define _MTLFX_PRIVATE_VISIBILITY                       __attribute__( ( visibility("default" ) , dllexport ) )
...
#define MTLFX_DEF_FUNC( name, signature )               using Fn##name = signature; \
                                                        Fn##name name = reinterpret_cast< Fn##name >( unix_dlsym( 0, #name ) )
...
namespace MTLFX::Private
{
    ...
    {
        const _Type* pAddress = static_cast<_Type*>(unix_dlsym(0, pSymbol));
